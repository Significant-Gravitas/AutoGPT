...
4. Rename `.env.template` to `.env` and fill in your `OPENAI_API_KEY`. If you plan to use Speech Mode, fill in your `ELEVEN_LABS_API_KEY` as well.
  ...
  - If you want to use GPT on an Azure instance, set `USE_AZURE` to `True` and provide the `OPENAI_API_BASE`, `OPENAI_API_VERSION` and `FAST_LLM_MODEL_DEPLOYMENT_ID`/`SMART_LLM_MODEL_DEPLOYMENT_ID` (since each deployment is created according to one specific model) values as explained here: https://pypi.org/project/openai/ in the `Microsoft Azure Endpoints` section
...
*Remember that your free daily custom search quota is 100 searces. If you want to increase this limit, you need to assign a billing account to the project to profit from up to 10K daily searches.*
