...
report_interrupted_migration() {
  ...
  log ""
  log "If the instance already holds data, restore your backup, or determine"
  log "whether that migration's changes reached the database and record the"
  log "answer before restarting -- from a shell with PostgreSQL running:"
  log "  cd ${AUTOGPT_BACKEND_DIR}"
