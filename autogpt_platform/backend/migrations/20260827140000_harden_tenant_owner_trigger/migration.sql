...
BEGIN
    ...
    PERFORM 1 FROM "Organization" WHERE id = owner_org_id FOR SHARE;
    IF NOT EXISTS (
        SELECT 1
    ...
    IF owner_team_id IS NOT NULL THEN
        PERFORM 1 FROM "Team"
        WHERE id = owner_team_id AND "orgId" = owner_org_id
        FOR SHARE;
    END IF;
    ...
    IF TG_TABLE_NAME = 'StoreListing' AND NOT EXISTS (
        ...
        WHERE id = graph_lock_id AND "organizationId" = owner_org_id
    ) THEN
...
BEGIN
    EXECUTE format(
        'ALTER FUNCTION %I.enforce_live_tenant_resource_owner() '
