...
export default function () {
  ...
  try {
    ...
    if (!headers || !headers.Authorization) {
      console.log(`⚠️ VU ${__VU} has no valid pre-authentication token - skipping iteration`);
      check(null, {
    ...
    const authCheck = check(headers, {
      'Authentication: Pre-auth token available': (h) => h && h.Authorization && h.Authorization.length > 0,
    });
