...
class GithubSponsorshipTriggerBlock(GitHubTriggerBase, Block):
    """Trigger block for GitHub Sponsors events - thank and engage with sponsors."""

    EXAMPLE_PAYLOAD_FILE = (
        Path(__file__).parent / "example_payloads" / "sponsorship.created.json"
    )
    ...
    class Input(GitHubTriggerBase.Input):
        class EventsFilter(BaseModel):
            ...
            pending_tier_change: bool = False

        events: EventsFilter = SchemaField(
            title="Events", description="The sponsorship events to subscribe to"
        )

    class Output(GitHubTriggerBase.Output):
        event: str = SchemaField(
            description="The sponsorship event that triggered the webhook"
        )
        sponsorship: dict = SchemaField(description="The full sponsorship object")
        sponsor: dict = SchemaField(description="The user who is sponsoring")
        sponsorable: dict = SchemaField(
            description="The user/org receiving the sponsorship"
        )
        tier: dict = SchemaField(description="The sponsorship tier object")
        tier_name: str = SchemaField(description="Name of the sponsorship tier")
        privacy_level: str = SchemaField(
            description="Privacy level ('public' or 'private')"
        )
        is_one_time: bool = SchemaField(
            description="Whether this is a one-time payment"
        )
    ...
    def __init__(self):
        ...
        super().__init__(
            id="584cca95-0b4f-4a99-9832-55b485ef6dc9",
            description="This block triggers on GitHub Sponsors events. "
            "Perfect for thanking sponsors, sending welcome messages, or unlocking perks.",
            categories={BlockCategory.DEVELOPER_TOOLS, BlockCategory.INPUT},
            input_schema=GithubSponsorshipTriggerBlock.Input,
            output_schema=GithubSponsorshipTriggerBlock.Output,
            webhook_config=BlockWebhookConfig(
                provider=ProviderName.GITHUB,
                webhook_type=GithubWebhookType.REPO,
                resource_format="{repo}",
                event_filter_input="events",
                event_format="sponsorship.{event}",
            ),
            test_input={
                "repo": "Significant-Gravitas/AutoGPT",
                "events": {"created": True},
            ...
            test_output=[
                ...
                ("triggered_by_user", example_payload["sender"]),
                ("event", example_payload["action"]),
                ("sponsorship", example_payload["sponsorship"]),
                ("sponsor", example_payload["sponsorship"]["sponsor"]),
                ("sponsorable", example_payload["sponsorship"]["sponsorable"]),
                ("tier", example_payload["sponsorship"]["tier"]),
                ("tier_name", example_payload["sponsorship"]["tier"]["name"]),
                ("privacy_level", example_payload["sponsorship"]["privacy_level"]),
                ("is_one_time", example_payload["sponsorship"]["tier"]["is_one_time"]),
            ],
        )
    ...
    async def run(self, input_data: Input, **kwargs) -> BlockOutput:  # type: ignore
        async for name, value in super().run(input_data, **kwargs):
            yield name, value
        sponsorship = input_data.payload["sponsorship"]
        yield "event", input_data.payload["action"]
        yield "sponsorship", sponsorship
        yield "sponsor", sponsorship["sponsor"]
        yield "sponsorable", sponsorship["sponsorable"]
        yield "tier", sponsorship["tier"]
        yield "tier_name", sponsorship["tier"]["name"]
        yield "privacy_level", sponsorship["privacy_level"]
        yield "is_one_time", sponsorship["tier"]["is_one_time"]
