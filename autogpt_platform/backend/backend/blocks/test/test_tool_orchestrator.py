...
async def test_graph_validation_with_tool_nodes_correct(server: SpinTestServer):
    ...
    nodes = [
        graph.Node(
            block_id=ToolOrchestratorBlock().id,
            input_default={
                "prompt": "Hello, World!",
                "credentials": creds,
            },
...
@pytest.mark.asyncio(loop_scope="session")
async def test_tool_orchestrator_function_signature(server: SpinTestServer):
    from backend.blocks.agent import AgentExecutorBlock
    ...
    tool_functions = await ToolOrchestratorBlock._create_tool_node_signatures(
...
@pytest.mark.asyncio
async def test_tool_orchestrator_tracks_llm_stats():
    """Test that ToolOrchestratorBlock correctly tracks LLM usage stats."""
    import backend.blocks.llm as llm_module
    from backend.blocks.tool_orchestrator import ToolOrchestratorBlock
...
@pytest.mark.asyncio
async def test_tool_orchestrator_parameter_validation():
    """Test that ToolOrchestratorBlock correctly validates tool call parameters."""
    import backend.blocks.llm as llm_module
    from backend.blocks.tool_orchestrator import ToolOrchestratorBlock
...
@pytest.mark.asyncio
async def test_tool_orchestrator_raw_response_conversion():
    """Test that ToolOrchestrator correctly handles different raw_response types with retry mechanism."""
    import backend.blocks.llm as llm_module
    from backend.blocks.tool_orchestrator import ToolOrchestratorBlock
...
@pytest.mark.asyncio
async def test_tool_orchestrator_agent_mode():
...
@pytest.mark.asyncio
async def test_tool_orchestrator_traditional_mode_default():
...
@pytest.mark.asyncio
async def test_tool_orchestrator_uses_customized_name_for_blocks():
    """Test that ToolOrchestratorBlock uses customized_name from node metadata for tool names."""
    ...
    result = await ToolOrchestratorBlock._create_block_function_signature(
        mock_node, [mock_link]
    )
...
@pytest.mark.asyncio
async def test_tool_orchestrator_falls_back_to_block_name():
    """Test that ToolOrchestratorBlock falls back to block.name when no customized_name."""
    from unittest.mock import MagicMock
...
@pytest.mark.asyncio
async def test_tool_orchestrator_uses_customized_name_for_agents():
    """Test that ToolOrchestratorBlock uses customized_name from metadata for agent nodes."""
    from unittest.mock import AsyncMock, MagicMock, patch
    ...
    with patch(
        ...
        result = await ToolOrchestratorBlock._create_agent_function_signature(
            mock_node, [mock_link]
        )
...
@pytest.mark.asyncio
async def test_tool_orchestrator_agent_falls_back_to_graph_name():
