...
class TestOpenRouterAliases:
    ...
    def test_openrouter_reverse_length_matches_aliases(self) -> None:
        """Dict inversion silently drops duplicate values; assert 1:1."""
        assert len(llm._OPENROUTER_REVERSE) == len(llm._OPENROUTER_ALIASES)
    ...
    def test_openrouter_reverse_fallback_for_non_aliased_model(
    ...
    def test_openrouter_dispatch_uses_reverse_mapping(self, mocker):
        ...
        This is a behavioural regression test for the dispatch line that
        converts an ``LlmModel`` back to an OpenRouter slug.  If the line
        is refactored (e.g. the ``get`` call is removed), this test will
        catch the regression because the mocked client will receive the
        bare enum value instead of the prefixed slug.
        """
        import openai

        fake_create = mocker.patch(
            "openai.AsyncOpenAI.chat.completions.create",
            new_callable=mocker.AsyncMock,
        )
        fake_create.return_value = mocker.Mock(
            choices=[mocker.Mock(message=mocker.Mock(content="hi"))],
            usage=mocker.Mock(prompt_tokens=1, completion_tokens=1),
        )

        # Call the internal helper that dispatches to OpenRouter
        result = asyncio.run(
            llm._llm_call(
                prompt=[{"role": "user", "content": "hello"}],
                model=llm.LlmModel.CLAUDE_4_7_OPUS,
                api_key="sk-test",
                provider="open_router",
            )
        )

        # The critical assertion: OpenRouter must see the prefixed slug
        call_kwargs = fake_create.call_args.kwargs
        assert call_kwargs["model"] == "anthropic/claude-opus-4-7"

    def test_openrouter_dispatch_fallback_for_non_aliased_model(self, mocker):
        """Models without a reverse entry must use their bare enum value."""
        import openai
        ...
        assert fake_create.call_args.kwargs["model"] == llm.LlmModel.GPT_4O.value
