...
@pytest.mark.asyncio
async def test_orchestrator_tool_execution_passes_complete_input_data():
    """Regression test: all fields from merged_input_data must be passed to on_node_execution.

    Previously, the loop in _execute_single_tool_with_manager called
    upsert_execution_input for each field but only stored the *last*
    field's value in `inputs`.  This caused `validate_exec` to fail with
    "missing input {'credentials'}" when credentials was not the last
    field in merged_input_data.

    See: https://github.com/Significant-Gravitas/AutoGPT/issues/...
    """
    import backend.blocks.llm as llm_module
    from backend.blocks.orchestrator import OrchestratorBlock
    ...
    mock_tool_call.function.name = "ai_text_generator"
    # Only "prompt" is provided by the LLM; "credentials" lives in input_default
    mock_tool_call.function.arguments = '{"prompt": "Hello, world!"}'
    ...
    llm_call_mock = AsyncMock(return_value=mock_response)

    # --- Tool signature with two fields ---
    mock_tool_signatures = [
        {
            "type": "function",
            "function": {
                "name": "ai_text_generator",
                "_sink_node_id": "tool-node-id",
                "_field_mapping": {},
                "parameters": {
                    "properties": {
                        "prompt": {"type": "string"},
    ...
    # --- Mock db client: return a node with credentials in input_default ---
    mock_db_client = AsyncMock()
    mock_target_node = MagicMock()
    mock_target_node.block_id = "ai-text-generator-block-id"
    # "credentials" is in input_default (not emitted by the LLM)
    ...
    # upsert_execution_input returns (NodeExecutionResult, accumulated_input_dict)
    mock_node_exec_result = MagicMock()
    mock_node_exec_result.node_exec_id = "tool-exec-id"
    mock_db_client.upsert_execution_input.return_value = (
        mock_node_exec_result,
        None,  # second return value is not used by the fixed code
    )
    ...
    with (
        ...
        input_data = OrchestratorBlock.Input(
            prompt="Call the AI text generator",
            model=llm_module.DEFAULT_LLM_MODEL,
            credentials=llm_module.TEST_CREDENTIALS_INPUT,  # type: ignore
        ...
        # Verify on_node_execution was called
        assert mock_execution_processor.on_node_execution.call_count == 1

        # Extract the NodeExecutionEntry passed to on_node_execution
        call_kwargs = mock_execution_processor.on_node_execution.call_args
        node_exec_entry = call_kwargs.kwargs["node_exec"]

        # The key assertion: `inputs` must contain BOTH fields from
        # merged_input_data ({**input_default, **raw_input_data}).
        # Before the fix, only the last field ("model") was passed because
        # the loop overwrote final_input_data on every iteration.
        assert "prompt" in node_exec_entry.inputs, (
            f"'prompt' missing from inputs. Got: {node_exec_entry.inputs!r}"
        )
        assert "credentials" in node_exec_entry.inputs, (
            f"'credentials' missing from inputs. Got: {node_exec_entry.inputs!r}"
        )
        assert node_exec_entry.inputs["prompt"] == "Hello, world!"
        assert node_exec_entry.inputs["credentials"] == {
            "id": "cred-123",
            "provider": "openai",
            "type": "api_key",
            "title": "Test Key",
