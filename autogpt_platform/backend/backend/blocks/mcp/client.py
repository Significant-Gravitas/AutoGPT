...
def normalize_mcp_authorization(value: str) -> str:
    ...
    Accepts a bare token (sent as Bearer for backward compatibility), an
    explicit ``Basic``/``Bearer`` prefix, or a complete ``Authorization:``
    header.  Runs once, where user input enters the system; stored values go
    through ``helpers.mcp_authorization_header`` and are never re-parsed.
    """
    ...
    if scheme == "Basic":
        # RFC 7617: the wire value is Base64 of user:password, so neither
        # character can appear in a valid credential.
        if ":" in credential:
            raise ValueError(
...
class MCPClient:
    ...
    ``authorization`` is sent verbatim as the ``Authorization`` header; callers
    pass an already-canonical value.
    """
    ...
    async def discover_auth_server_metadata(
        ...
        Given an authorization server URL, returns the metadata dict, which
        holds ``authorization_endpoint``, ``token_endpoint``,
        ``registration_endpoint`` (for dynamic client registration),
        ``scopes_supported``, ``code_challenge_methods_supported`` and so on,
        paired with the issuer identifier that document is *required* to
        declare.

        RFC 8414 §3.3 ties the two together: the ``issuer`` in the document
        must be the URL the document was fetched for, minus the well-known
        path segment.  Which of the candidate URLs below answered therefore
        decides the expected issuer, and only the caller comparing against
        that exact value can tell a tenant apart from its own origin.
        """
        from urllib.parse import urlparse
        ...
        # Try standard metadata endpoints (RFC 8414 and OpenID Connect), each
        # paired with the issuer a document served there must declare.
        candidates = []
        if path and path != "/":
            candidates.append(
                (
