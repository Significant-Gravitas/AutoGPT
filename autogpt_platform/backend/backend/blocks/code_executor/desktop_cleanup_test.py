...
from backend.blocks.code_executor.desktop import kill_desktop_sandbox
...
async def test_cancelled_cleanup_continues_until_kill_finishes():
    ...
    with patch("backend.blocks.code_executor.desktop.AsyncSandbox.kill", kill):
...
async def test_hung_cleanup_times_out_and_logs_reason(caplog):
    ...
    with (
        patch("backend.blocks.code_executor.desktop.AsyncSandbox.kill", kill),
        patch("backend.blocks.code_executor.desktop.CLEANUP_TIMEOUT", 0.01),
    ):
