...
StoreOperation = Literal["add_to_library", "get_agent_details", "list_store_agents", "search_store"]
...
class AddToLibraryFromStoreBlock(Block):
    ...
    class Output(BlockSchema):
        ...
        agent_id: str = SchemaField(
            description="The ID of the agent graph"
        )
...
class GetStoreAgentDetailsBlock(Block):
    ...
    class Input(BlockSchema):
        slug: str = SchemaField(
            description="The slug identifier of the store agent"
        )
    ...
    class Output(BlockSchema):
        ...
        runs: int = SchemaField(
            description="Number of times the agent has been run",
            default=0
        )
    ...
    async def run(
        ...
        **kwargs,
        ...
        try:
            ...
            yield "store_listing_version_id", details.get("store_listing_version_id", "")
    ...
    async def _get_agent_details(
        ...
        try:
            if version:
                ...
                parts = slug.split("/")
                if len(parts) != 2:
                    raise ValueError(f"Invalid slug format: {slug}. Expected format: username/agent-name")
            ...
            return {
                ...
                "categories": agent_details.categories if hasattr(agent_details, 'categories') else [],
