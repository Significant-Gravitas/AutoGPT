...
from backend.executor.auto_credentials import parse_auto_credential_field
    ...
    for node in graph.nodes:
        ...
        if auto_credentials_fields:
            for _kwarg_name, info in auto_credentials_fields.items():
                ...
                field_present = field_name in node.input_default
                if nodes_input_masks and node.id in nodes_input_masks:
                    node_input_mask = nodes_input_masks[node.id]
                    if field_name in node_input_mask:
                        field_value = node_input_mask[field_name]
                        field_present = True
                ...
                parsed = parse_auto_credential_field(
                    ...
                    field_present_in_input=field_present,
                )
                ...
                if parsed.invalid_value_type:
                    ...
                    has_missing_credentials = True
                    credential_errors[node.id][field_name] = (
                        f"{CRED_ERR_NOT_AVAILABLE_PREFIX} invalid "
                        f"{parsed.invalid_value_type} value for this field. "
                        "Please re-select the file."
                    )
                    continue
                ...
                try:
                    ...
                    creds = await creds_store.get_creds_by_id(user_id, parsed.cred_id)
                except Exception as e:
                    if field_is_optional:
                        _mark_optional_skip()
                        continue
                ...
                if not creds:
                    ...
                    credential_errors[node.id][
                        field_name
                    ] = f"{CRED_ERR_UNKNOWN_PREFIX}{parsed.cred_id}"
