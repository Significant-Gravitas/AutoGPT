...
async def test_agent_schedule_validation(server: SpinTestServer):
    ...
    with pytest.raises(ValueError, match="Cron expression cannot be empty"):
        await scheduler.add_execution_schedule(
            graph_id=test_graph.id,
            user_id=test_user.id,
            graph_version=1,
            cron="",
    ...
    # Test whitespace-only cron expression
    with pytest.raises(ValueError, match="Cron expression cannot be empty"):
        await scheduler.add_execution_schedule(
            graph_id=test_graph.id,
            user_id=test_user.id,
    ...
    # Test invalid cron expression format (wrong number of fields)
    with pytest.raises(ValueError, match="Invalid cron expression"):
        await scheduler.add_execution_schedule(
            graph_id=test_graph.id,
            user_id=test_user.id,
            graph_version=1,
            cron="0 0 * *",  # Only 4 fields instead of 5
            input_data={"input": "data"},
            input_credentials={},
        )
