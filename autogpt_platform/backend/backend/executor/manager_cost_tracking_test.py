...
class TestResolveTracking:
    ...
    def test_unreal_speech_empty_text(self):
        stats = self._stats()
        tt, amt = resolve_tracking("unreal_speech", "TTSBlock", stats, {"text": ""})
        assert tt == "characters"
        assert amt == 0.0

    def test_unreal_speech_non_string_text(self):
        stats = self._stats()
        tt, amt = resolve_tracking("unreal_speech", "TTSBlock", stats, {"text": 123})
        assert tt == "characters"
        assert amt == 0.0
    ...
    def test_d_id_uses_script_input(self):
        stats = self._stats()
        tt, amt = resolve_tracking(
            "d_id", "TalkingHead", stats, {"script_input": "Hello"}
        )
        assert tt == "characters"
        assert amt == 5.0
    ...
    def test_elevenlabs_uses_text(self):
        stats = self._stats()
        tt, amt = resolve_tracking(
            "elevenlabs", "TTSBlock", stats, {"text": "Say this"}
        )
        assert tt == "characters"
        assert amt == 8.0
    ...
    def test_elevenlabs_fallback_to_text_when_no_script_input(self):
        stats = self._stats()
        tt, amt = resolve_tracking(
            "elevenlabs", "TTSBlock", stats, {"text": "Fallback text"}
        )
        assert tt == "characters"
        assert amt == 13.0
    ...
    def test_e2b_returns_sandbox_seconds(self):
        ...
        tt, amt = resolve_tracking("e2b", "CodeBlock", stats, {})
    ...
    def test_tokens_take_precedence_over_provider_specific(self):
        ...
        tt, amt = resolve_tracking("fal", "FalBlock", stats, {})
        assert tt == "tokens"
        assert amt == 100.0
