...
class ReadWorkspaceFileInput(BaseModel):
    ...
    @field_validator("file_id", "path", "save_to_path", mode="before")
    @classmethod
    def strip_optional_strings(cls, v: Any) -> str | None:
        return v.strip() if isinstance(v, str) else v
...
class WriteWorkspaceFileInput(BaseModel):
    ...
    @field_validator(
        "source_path", "content", "content_base64", "path", "mime_type", mode="before"
    )
    @classmethod
    def strip_optional_strings(cls, v: Any) -> str | None:
        return v.strip() if isinstance(v, str) else v
    ...
    async def _execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            scope = "all sessions" if params.include_all_sessions else "current session"
...
class ReadWorkspaceFileTool(BaseTool):
    ...
    async def _execute(
        ...
        **kwargs: Any,
        ...
        char_offset: int = max(0, params.offset)
        char_length: Optional[int] = params.length
        ...
        try:
            ...
            if is_small and (is_text or is_image) and not params.force_download_url:
