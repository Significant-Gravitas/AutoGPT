...
workspace).  The model has a root shell in that same VM, so the desktop is
shared with it, never private from it: any account a person signs into on
it is readable by the model.  The box pauses at turn end like always and
the screen comes back exactly as it was on the next resume.
"""
