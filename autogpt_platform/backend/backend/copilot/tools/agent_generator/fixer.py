...
# Defaults applied to ToolOrchestratorBlock nodes by the fixer.
...
class AgentFixer:
    ...
    def fix_tool_orchestrator_blocks(self, agent: AgentDict) -> AgentDict:
        """Fix ToolOrchestratorBlock nodes to ensure agent-mode defaults.
        ...
        for node in nodes:
            ...
            for field, default_value in _SDM_DEFAULTS.items():
                if field not in input_default or input_default[field] is None:
                    ...
                    self.add_fix_log(
                        f"ToolOrchestratorBlock {node_id}: "
    ...
    def apply_all_fixes(
        ...
        # Apply fixes for ToolOrchestratorBlock nodes (agent-mode defaults)
        agent = self.fix_tool_orchestrator_blocks(agent)
