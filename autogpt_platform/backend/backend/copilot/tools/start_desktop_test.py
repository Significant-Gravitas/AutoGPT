...
_STREAM = DesktopStream(
    url="https://6080-sbx.e2b.app/vnc.html?autoconnect=true&password=secret",
    sandbox_id=_BOX,
)
...
def _preview_link(user_id: str, live_url: str) -> str:
    return f"/api/proxy/api/desktop-preview?for={user_id}"


@pytest.fixture(autouse=True)
def _owner_bound_links():
    """The real link needs a Fernet key; stub the wrapper."""
    with patch(f"{_C}.create_preview_link", side_effect=_preview_link):
        yield
...
class _Box:
    ...
    def display_flag(self):
        return [
            c for c in self.redis.set.await_args_list if c.args[0].endswith(":display")
        ]
...
class TestStartDesktop:
    ...
    async def test_turns_the_screen_on_in_the_sessions_box(self):
        ...
        assert box.display_flag()[0].args[:2] == (
    ...
    async def test_stored_result_carries_the_owner_link_not_the_password(self):
        ...
        assert "vnc.html" not in result.model_dump_json()
    ...
    async def test_no_user_id_gets_no_desktop(self):
        ...
        issue it to, so the box is not even looked up."""
        tool = StartDesktopTool()
        session = make_session(user_id=_USER)
        ...
        box.get_box.assert_not_awaited()
...
class TestExpertDesktop:
    ...
    async def test_expert_screen_is_keyed_by_expert_with_home_and_shared(self):
        ...
        flag_key = box.display_flag()[0].args[0]
        assert flag_key == f"copilot:e2b:expert:{self._EXPERT}:shell:display"
        assert session.session_id not in flag_key
