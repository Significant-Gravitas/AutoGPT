...
@pytest.mark.asyncio
async def test_blocked_when_library_check_missing(tool: DecomposeGoalTool):
    """decompose_goal must refuse to run until find_library_agent(for_creation=true)
    has been called in the current turn, so the build plan can't render before
    matches are surfaced to the user."""
    session = make_session(_USER_ID, library_check=False)
...
@pytest.mark.asyncio
async def test_runs_when_library_check_satisfied(tool: DecomposeGoalTool, session):
    """The default make_session announces an in-flight
    find_library_agent(for_creation=true), so decompose_goal proceeds. Pinned
    here as a named test so the gate's pass path is documented separately
    from the generic happy-path coverage."""
    result = await tool._execute(
        user_id=_USER_ID,
        session=session,
        goal="Build a news summarizer agent",
...
@pytest.mark.asyncio
async def test_builder_bound_session_bypasses_gate(tool: DecomposeGoalTool):
    """Builder-bound sessions (user is already editing a specific agent)
    bypass the library-similarity gate, matching require_library_check's
    builder_graph_id short-circuit."""
    session = make_session(_USER_ID, library_check=False)
