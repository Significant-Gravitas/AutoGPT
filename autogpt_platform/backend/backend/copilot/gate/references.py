...
_Found = tuple[str, str | None] | None
...
async def _resolve(ref: Reference, call: _Call) -> Reference:
    ...
    if found is None or not found[0].strip():
        return ref
    name, href = found
    return ref.model_copy(update={"name": " ".join(name.split()), "href": href})


async def _library_folder(folder_id: str, call: _Call) -> _Found:
    folder = await library_db().get_folder(folder_id, call.user_id)
    return folder.name, f"/library?{urlencode({'folder': folder.id})}"


async def _library_agent(agent_id: str, call: _Call) -> _Found:
    agent = await library_db().get_library_agent(agent_id, call.user_id)
    return agent.name, _agent_href(agent.id)


async def _graph(graph_id: str, call: _Call) -> _Found:
    agent = await library_db().get_library_agent_by_graph_id(call.user_id, graph_id)
    return (agent.name, _agent_href(agent.id)) if agent else None


async def _agent_or_graph(agent_id: str, call: _Call) -> _Found:
    return await _graph(agent_id, call) or await _library_agent(agent_id, call)


async def _preset(preset_id: str, call: _Call) -> _Found:
    ...
    return preset.name, _agent_href(agent.id, activeTab=tab, activeItem=item)


async def _schedule(schedule_id: str, call: _Call) -> _Found:
    ...
    if isinstance(job, GraphExecutionJobInfo):
        ...
        if agent is not None:
            return job.name, _agent_href(
                agent.id, activeTab="scheduled", activeItem=schedule_id
            )
    return job.name, "/library/followups"


async def _chat_session(session_id: str, call: _Call) -> _Found:

    meta = await get_chat_session_metadata(session_id, call.user_id)
    if meta is None:
        return None
    return (
        meta.title or "Untitled chat",
        f"/copilot?{urlencode({'sessionId': session_id})}",
    )


async def _expert(expert_id: str, call: _Call) -> _Found:
    expert = await experts_db().get_expert(
        call.user_id, expert_id, include_workflows=False
    )
    return (expert.name, _team_href(expert.id)) if expert else None


async def _expert_or_name(reference: str, call: _Call) -> _Found:
...
async def _expert_template(template_id: str, call: _Call) -> _Found:
    templates = await experts_db().list_templates()
    template = next((t for t in templates if t.id == template_id), None)
    if template is None:
        return None
    return template.name, f"/marketplace/experts/{quote(template.id, safe='')}"


async def _expert_workflow(workflow_id: str, call: _Call) -> _Found:
    label = await experts_db().get_workflow_label(call.user_id, workflow_id)
    if label is None or label.name is None:
        return None
    return label.name, _team_href(label.expert_id)


async def _credential(credential_id: str, call: _Call) -> _Found:
    ...
    if creds is None:
        return None
    return creds.title or str(creds.provider), "/settings/integrations"


async def _routine(routine_id: str, call: _Call) -> _Found:
    ...
    if routine is None:
        return None
    return routine.title, _team_href(routine.expert_id) if routine.expert_id else None


async def _store_listing(version_id: str, call: _Call) -> _Found:
    agent = await store_db().get_store_agent_by_version_id(version_id)
    return (
        agent.agent_name,
        f"/marketplace/agent/{quote(agent.creator, safe='')}/"
        f"{quote(agent.slug, safe='')}",
    )


async def _workspace_file(file_id: str, call: _Call) -> _Found:
    ...
    return file.name, f"/artifacts{query}"


async def _team_change(confirmation_id: str, call: _Call) -> _Found:
    ...
    if proposal.expert_id:
        return proposal.preview.name, _team_href(proposal.expert_id)
    return proposal.preview.name, None


async def _soul_change(confirmation_id: str, call: _Call) -> _Found:
...
_RESOLVERS: dict[Entity, Callable[[str, _Call], Awaitable[_Found]]] = {
