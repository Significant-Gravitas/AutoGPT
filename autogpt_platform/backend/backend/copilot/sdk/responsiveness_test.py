...
testable) and the adaptive thinking ``effort`` resolution from the
configured override.
"""
...
# ``stream_chat_completion_sdk``).  A regression here means the SDK
# stops honouring the operator's static
# ``CHAT_CLAUDE_AGENT_THINKING_EFFORT`` override.
# ---------------------------------------------------------------------------


def _resolve_effort(configured: str | None) -> str:
    """Mirror the inline ``effort=`` expression so the default is unit-tested
...
class TestAdaptiveEffortResolution:
    """Verify the ``effort`` value chosen for each config value.

    This tests the formula used at the ``ClaudeAgentOptions`` call site:
        ``config_value or "high"``
    """

    def test_honours_config_override(self):
        """A configured effort is respected."""
        assert _resolve_effort(configured="low") == "low"
        assert _resolve_effort(configured="max") == "max"
        assert _resolve_effort(configured="medium") == "medium"

    def test_defaults_to_high_when_unconfigured(self):
        """No config override → 'high' (the responsive default for SDK turns)."""
        assert _resolve_effort(configured=None) == "high"
