...
def test_try_parse_structured_json_array():
    """JSON arrays are parsed into Python lists."""
    result = _try_parse_structured('[["Name","Score"],["Bob","90"]]')
    assert result == [["Name", "Score"], ["Bob", "90"]]


def test_try_parse_structured_json_object():
    """JSON objects are parsed into Python dicts."""
    result = _try_parse_structured('{"key": "value", "num": 42}')
    assert result == {"key": "value", "num": 42}


def test_try_parse_structured_csv():
    """CSV content is parsed into a list of lists."""
    csv_content = "Name,Score,Grade\nAlice,95,A\nBob,85,B\n"
    result = _try_parse_structured(csv_content)
    assert result == [
        ["Name", "Score", "Grade"],
        ["Alice", "95", "A"],
        ["Bob", "85", "B"],
...
def test_try_parse_structured_tsv():
    """Tab-separated content is parsed as CSV."""
    tsv_content = "Name\tScore\nAlice\t95\nBob\t85\n"
    result = _try_parse_structured(tsv_content)
    assert result == [
        ["Name", "Score"],
        ["Alice", "95"],
        ["Bob", "85"],
...
def test_try_parse_structured_plain_text():
    """Plain text without structure is returned unchanged."""
    text = "This is just a plain text document with no structure."
    assert _try_parse_structured(text) == text


def test_try_parse_structured_single_line():
    """Single-line text with a comma is not treated as CSV (needs >= 2 rows)."""
    text = "hello, world"
    assert _try_parse_structured(text) == text


def test_try_parse_structured_empty():
    """Empty string is returned unchanged."""
    assert _try_parse_structured("") == ""


def test_try_parse_structured_json_string():
    """A bare JSON string is parsed."""
    result = _try_parse_structured('"hello"')
    assert result == "hello"


# ---------------------------------------------------------------------------
# expand_file_refs_in_args — structured data auto-parsing
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_expand_args_bare_ref_json_array():
    """A bare @@agptfile ref resolving to JSON is parsed into a Python list."""

    async def _resolve_json(ref, _uid, _sess):
        return '[["Name","Score"],["Bob","90"]]'

    with patch(
        "backend.copilot.sdk.file_ref.resolve_file_ref",
        new=AsyncMock(side_effect=_resolve_json),
    ):
        result = await expand_file_refs_in_args(
            {"values": "@@agptfile:workspace://data-file"},
            user_id="u1",
            session=_make_session(),
        )
    assert result["values"] == [["Name", "Score"], ["Bob", "90"]]


@pytest.mark.asyncio
async def test_expand_args_bare_ref_csv():
    """A bare @@agptfile ref resolving to CSV is parsed into list[list[str]]."""

    async def _resolve_csv(ref, _uid, _sess):
        return "Name,Score,Grade\nAlice,95,A\nBob,85,B\n"

    with patch(
        "backend.copilot.sdk.file_ref.resolve_file_ref",
        new=AsyncMock(side_effect=_resolve_csv),
    ):
        result = await expand_file_refs_in_args(
            {"values": "@@agptfile:workspace://csv-file"},
            user_id="u1",
            session=_make_session(),
        )
...
@pytest.mark.asyncio
async def test_expand_args_embedded_ref_stays_string():
    """An embedded ref (not bare) always expands to a string, even if content is JSON."""
    ...
    with patch(
        ...
        result = await expand_file_refs_in_args(
            {"prompt": "Data: @@agptfile:workspace://data then done"},
            user_id="u1",
            session=_make_session(),
        )
    # Embedded refs are always strings.
    assert result["prompt"] == 'Data: ["a","b"] then done'
