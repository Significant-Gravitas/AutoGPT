...
check it before adding or deleting one. `list_schedules` (optionally filtered
by `graph_id`) and `delete_schedule` manage schedules — change a cron by
deleting and re-creating via `run_agent`. **Note**: editing a trigger agent
makes a new version, but the old schedule keeps running the old one — delete
and re-create the schedule after editing.
