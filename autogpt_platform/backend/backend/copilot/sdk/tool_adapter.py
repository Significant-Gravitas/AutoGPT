        ...
        async def wrapper(args: dict[str, Any]) -> dict[str, Any]:
            ...
            if not args and input_schema and input_schema.get("required"):
                ...
                return _mcp_error(
                    ...
                    f"To fix this:\n"
                    f"1. For large file writes: first write the content "
                    f"to a workspace file using @@agptfile references "
                    f"(e.g. write to workspace then pass @@agptfile:filename), "
                    f"or write in smaller chunks using bash_exec with "
                    f"cat >> to append section by section.\n"
                    f"2. For long commands: simplify or split into "
                    f"multiple shorter calls.\n"
