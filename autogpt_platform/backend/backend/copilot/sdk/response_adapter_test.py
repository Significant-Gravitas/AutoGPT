...
def test_stream_event_summary_emits_text_when_only_thinking_streamed_partial():
    """If ``StreamEvent`` streamed only thinking, the summary's ``TextBlock``
    must still reach the wire.  Otherwise a CLI / OpenRouter quirk that
    delivers text only in the summary silently truncates the response.
    """
    adapter = _adapter()
    # Partial stream: thinking only
    adapter.convert_message(
        _stream_event(
            {
                "type": "content_block_start",
        ...
        _stream_event(
            {
                "type": "content_block_delta",
                "index": 0,
                "delta": {"type": "thinking_delta", "thinking": "c" * 80},
            }
        )
    )
    adapter.convert_message(
    ...
    # Summary arrives with both ThinkingBlock (already streamed) and TextBlock
    # (never streamed via partial).  Text must emit; thinking must not.
    summary = AssistantMessage(
        content=[
            ThinkingBlock(thinking="c" * 80, signature="sig"),
    ...
    results = adapter.convert_message(summary)
    text_deltas = [r for r in results if isinstance(r, StreamTextDelta)]
    reasoning_deltas = [r for r in results if isinstance(r, StreamReasoningDelta)]
    assert len(text_deltas) == 1 and text_deltas[0].delta == "final answer"
    assert reasoning_deltas == []


def test_stream_event_summary_emits_thinking_when_only_text_streamed_partial():
    """Converse case: partial streamed text only, summary carries the
    thinking block — reasoning must reach the wire via the summary.
    """
    adapter = _adapter()
    adapter.convert_message(
        _stream_event(
    ...
    summary = AssistantMessage(
        content=[
            ThinkingBlock(thinking="summary-only thinking", signature="sig"),
            TextBlock(text="streamed text"),
        ],
        model="test",
    )
    results = adapter.convert_message(summary)
    ...
    reasoning_deltas = [r for r in results if isinstance(r, StreamReasoningDelta)]
    assert (
        len(reasoning_deltas) == 1
        and reasoning_deltas[0].delta == "summary-only thinking"
    )
    assert text_deltas == []
