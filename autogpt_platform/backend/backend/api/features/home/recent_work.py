...
from backend.util.tenancy_urls import (
    ORGANIZATION_HOME,
...
from .helpers import UNKNOWN_AGENT, AgentScopeMap, to_home_expert
from .models import (
    HomeBriefingOutcome,
    HomeRecentWork,
    HomeRecentWorkGroup,
...
ActorKey = tuple[
    Literal["expert", "workflow", "autopilot"], str, str | None, str | None
]
...
def compose_recent_work(
    *,
    ...
    for item in run_items:
        target = bucket(_run_key(item, exec_by_id[item.execution_id]))
...
def _run_key(item: BriefingRunItem, execution: GraphExecutionMeta) -> ActorKey:
    scope = (execution.organization_id, execution.team_id)
    if item.expert_id:
        return ("expert", item.expert_id, *scope)
    return ("workflow", item.graph_id, *scope)


def _event_key(
    event: ActivityEvent, exec_by_id: dict[str, GraphExecutionMeta]
) -> ActorKey:
    scope = (event.organization_id, event.team_id)
    if event.expert_id:
        return ("expert", event.expert_id, *scope)
    if event.graph_exec_id:
        ...
        if execution and execution.expert_id:
            return ("expert", execution.expert_id, *scope)
        # A run past the fetch limit still keys by its own execution, so two
        # unknown runs never collapse into one unnamed workflow.
        return (
            "workflow",
            execution.graph_id if execution else event.graph_exec_id,
            *scope,
        )
    return ("autopilot", "", *scope)


def _actor(
    key: ActorKey, expert_by_id: dict[str, Expert], agent_by_graph: AgentScopeMap
) -> HomeWorkActor:
    kind, ref, organization_id, team_id = key
    if kind == "expert":
    ...
    if kind == "workflow":
        agent = agent_by_graph.get(
            (ref, organization_id, team_id), agent_by_graph.get(ref, UNKNOWN_AGENT)
        )
        return HomeWorkActor(
            kind="workflow",
            name=agent.name,
            ...
            link=(
                library_agent_path(agent.library_agent_id, organization_id, team_id)
...
def _compose_item(
    ...
    return HomeRecentWorkItem(
        ...
        link=(
            copilot_path(session_id, event.organization_id, event.team_id)
            if session_id
            else None
        ),
    )
