...
def test_recent_work_keeps_same_graph_in_different_workspaces_separate() -> None:
    executions = [
        _execution(exec_id="run-a").model_copy(
            update={"organization_id": "org-1", "team_id": "team-a"}
        ),
        _execution(exec_id="run-b").model_copy(
            update={"organization_id": "org-1", "team_id": "team-b"}
        ),
    ]
    agents = {
        ("graph-notes", "org-1", "team-a"): AgentRef(
            name="Team A notes", library_agent_id="library-a"
        ),
        ("graph-notes", "org-1", "team-b"): AgentRef(
            name="Team B notes", library_agent_id="library-b"
        ),
    }
    recent = compose_recent_work(
        now=NOW,
    ...
    assert {group.actor.name for group in recent.groups} == {
        "Team A notes",
        "Team B notes",
    }
    for group in recent.groups:
        expected_team = "team-a" if group.actor.name == "Team A notes" else "team-b"
        assert f"teamId={expected_team}" in group.actor.link


def test_recent_work_file_link_reopens_the_event_workspace() -> None:
    event = _event(event_id="file-1", session_id="session-1").model_copy(
        update={"organization_id": "org-1", "team_id": "team-a"}
    )
    recent = compose_recent_work(
        now=NOW,
    ...
    assert recent.groups[0].items[0].link == (
        "/copilot?organizationId=org-1&teamId=team-a&sessionId=session-1"
    )
...
def test_groups_runs_and_deliverables_under_the_actor_that_did_them() -> None:
    ...
    assert (
        maria_group.actor.link
        == "/copilot?organizationId=__personal__&teamId=__org_home__&expertId=maria"
    )
    ...
    assert (
        maria_group.items[0].link
        == "/copilot?organizationId=__personal__&teamId=__org_home__&sessionId=s1"
    )

    assert notes_group.actor.kind == "workflow"
    assert notes_group.actor.image_url == "https://example.com/release-notes.png"
    assert (
        notes_group.actor.link
...
def test_thread_work_without_an_expert_is_autopilots() -> None:
    ...
    assert (
        group.items[0].link
        == "/copilot?organizationId=__personal__&teamId=__org_home__&sessionId=s2"
    )
