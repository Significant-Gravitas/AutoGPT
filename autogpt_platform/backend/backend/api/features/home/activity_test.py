...
def test_active_tasks_map_status_and_cap_the_list() -> None:
    ...
    assert tasks[0].link == (
        "/library/agents/library-agent?organizationId=__personal__&teamId=__org_home__&activeTab=runs&activeItem=running"
    )
...
def test_active_tasks_resolve_same_graph_by_exact_tenant() -> None:
    ...
    assert [(task.title, task.link) for task in tasks] == [
        (
            "Team A agent",
            "/library/agents/library-a?organizationId=org-1&teamId=team-a&activeTab=runs&activeItem=run-a",
        ),
        (
            "Team B agent",
            "/library/agents/library-b?organizationId=org-1&teamId=team-b&activeTab=runs&activeItem=run-b",
        ),
    ]
