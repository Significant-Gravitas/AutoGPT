...
from backend.copilot.idempotency import claim_stream_idempotency_key
...
class StreamChatRequest(BaseModel):
    ...
    idempotency_key: str | None = Field(
        ...
        max_length=64,
        description=(
            "Optional per-click UUID generated by the frontend.  Frontend / "
            "network / RMQ-redelivery retries of the **same** logical send "
            "reuse the same key; the backend dedups so only one user row + "
            "one assistant turn is created per key.  Distinct user clicks "
            "(even with identical text) MUST send different keys."
        ),
    )
...
async def stream_chat_post(
    ...
    # Pre-generate the turn_id so the idempotency-key claim records the
    # canonical turn for this logical send.  A redelivered POST that hits
    # an existing claim short-circuits before this turn_id is registered.
    turn_id = "" if (await is_turn_in_flight(session_id)) else str(uuid4())

    # Idempotency-key dedup (defence-in-depth on top of the trailing-same-role
    # check inside ``append_and_save_message``).  When the frontend supplies a
    # per-click UUID, claim it atomically in Redis before any DB work.  A
    # second POST carrying the same key — RMQ-redelivered task, browser/CDN
    # retry, or any infra-level duplicate — short-circuits to subscribing to
    ...
    if request.is_user_message and request.idempotency_key and turn_id:
        ...
        if existing_turn is not None:
            is_idempotency_dup = True
            logger.info(
                "[STREAM] idempotency-key hit for session=%s key=%s — "
                "subscribing to existing turn=%s",
                session_id,
                request.idempotency_key,
                existing_turn,
            )
    ...
    is_duplicate_message = is_idempotency_dup
    if not is_idempotency_dup and request.message:
        message = ChatMessage(
