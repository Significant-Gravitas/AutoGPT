...
def client(monkeypatch):
    @asynccontextmanager
    async def authorized_billing(*args, **kwargs):
        yield True

    monkeypatch.setattr(
        "backend.api.live_auth.live_org_permission_barrier", authorized_billing
    )
    app = FastAPI()
...
def test_revoked_billing_access_blocks_before_loading_history(client, monkeypatch):
    @asynccontextmanager
    async def revoked_billing(*args, **kwargs):
        yield False

    monkeypatch.setattr(
        "backend.api.live_auth.live_org_permission_barrier", revoked_billing
    )
    get_model = AsyncMock()
