...
@pytest.mark.asyncio
async def test_disabled_rollout_only_returns_the_owned_personal_organization(
    monkeypatch, mocker
):
    monkeypatch.setenv("FORCE_FLAG_SHOW_ORG_SETTINGS", "false")
    personal = MagicMock(id="own-personal-org")
    default = mocker.patch.object(
        routes.org_db,
        "get_user_default_team",
        AsyncMock(return_value=("own-personal-org", "default-team")),
    )
    ...
    get_org.assert_awaited_once_with("own-personal-org")
