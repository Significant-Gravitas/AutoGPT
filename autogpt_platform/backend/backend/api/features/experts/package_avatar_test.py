...
async def test_media_we_host_is_embedded_as_bytes(stored):
    ...
    avatar, content = await packaged_avatar(LOCAL, OWNER)
...
async def test_an_absolute_url_to_our_own_media_is_the_same_avatar(
    ...
    avatar, content = await packaged_avatar(f"https://app.example{LOCAL}", OWNER)
...
async def test_a_roster_path_travels_as_a_url():
    ...
    avatar, content = await packaged_avatar("/experts/maria.svg", OWNER)
...
async def test_media_we_do_not_host_is_dropped_rather_than_fetched(url: str):
    ...
    caller's choosing."""
    assert await packaged_avatar(url, OWNER) == (None, None)
...
async def test_a_format_we_cannot_name_is_dropped(stored):
    ...
    stored("a.bmp", b"BM")
    assert await packaged_avatar("/api/store/media/user-1/images/a.bmp", OWNER) == (
        None,
        None,
    )
...
async def test_an_avatar_over_the_cap_is_dropped(stored):
    ...
    assert await packaged_avatar(LOCAL, OWNER) == (None, None)
...
async def test_another_users_media_is_not_read_into_the_owners_package(stored):
    """Both media backends key an upload by its uploader, so the user segment of
    a stored avatarUrl is an authorization claim the owner writes for
    themselves. Packaging must not honour one that names somebody else."""
    stored("a.png", b"\x89PNG")

    assert await packaged_avatar(LOCAL, "user-2") == (None, None)


async def test_an_expert_with_no_owner_reads_no_user_media(stored):
    """A roster template has no owner, so there is no account whose media it
    could legitimately embed."""
    stored("a.png", b"\x89PNG")

    assert await packaged_avatar(LOCAL, None) == (None, None)


async def test_another_users_bucket_object_is_not_read_into_the_owners_package(
    mocker: pytest_mock.MockFixture,
):
    """The bucket check alone only proves the object is ours, not that it is
    theirs."""
    mocker.patch.object(
        package_avatar.settings.config, "media_gcs_bucket_name", "our-bucket"
    )
    download = mocker.patch.object(package_avatar.async_storage, "Storage")

    url = "https://storage.googleapis.com/our-bucket/users/user-2/images/a.png"
    assert await packaged_avatar(url, OWNER) == (None, None)
    download.assert_not_called()


async def test_the_owners_own_bucket_object_is_still_embedded(
    mocker: pytest_mock.MockFixture,
):
    client = mocker.AsyncMock()
    client.download.return_value = b"\x89PNG"
    storage = mocker.patch.object(package_avatar.async_storage, "Storage")
    storage.return_value.__aenter__.return_value = client
    mocker.patch.object(
        package_avatar.settings.config, "media_gcs_bucket_name", "our-bucket"
    )

    url = f"https://storage.googleapis.com/our-bucket/users/{OWNER}/images/a.png"
    avatar, content = await packaged_avatar(url, OWNER)

    assert avatar is not None and avatar.path == "avatar.png"
    assert content == b"\x89PNG"
    client.download.assert_awaited_once_with(
        "our-bucket", f"users/{OWNER}/images/a.png"
    )
