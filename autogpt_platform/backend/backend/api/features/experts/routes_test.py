...
@pytest.mark.parametrize("path", ["/experts", "/experts/pods", "/experts/identities"])
def test_expert_lists_are_empty_outside_the_personal_workspace(mocker, path):
    mocker.patch(
        "backend.api.features.experts.workspace.get_user_default_team",
        AsyncMock(return_value=("personal-org", "personal-team")),
    )
    list_roster = mocker.patch.object(
        experts_db, "list_experts", new_callable=AsyncMock
    )
    ...
    list_roster.assert_not_awaited()


@pytest.mark.parametrize(
    "path,payload,method",
    [
        ("/experts", {"template_id": "template-1"}, "hire_expert"),
        ("/experts/raise", {"name": "Sam"}, "create_raised_expert"),
    ],
)
def test_expert_creation_rejects_another_workspace(mocker, path, payload, method):
    mocker.patch(
        "backend.api.features.experts.workspace.get_user_default_team",
        AsyncMock(return_value=("personal-org", "personal-team")),
    )
    create = mocker.patch.object(experts_db, method, new_callable=AsyncMock)

    response = client.post(path, json=payload)

    assert response.status_code == 403
    assert response.json()["detail"]["code"] == "experts_personal_workspace_only"
...
def setup_app_auth(mock_jwt_user, mocker):
    ...
    mocker.patch(
        "backend.api.features.experts.workspace.get_user_default_team",
        AsyncMock(return_value=("test-org", "test-team")),
    )

    @asynccontextmanager
    async def allowed_barrier(*_args, **_kwargs):
        yield True

    mocker.patch(
        "backend.api.live_auth.live_resource_permission_barrier", allowed_barrier
    )
    yield
