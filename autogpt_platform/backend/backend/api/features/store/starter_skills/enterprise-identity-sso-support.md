...
The failure report with timestamps, the identity setup (IdP, app, SCIM
path), recent config changes, and the auth logs when available.
...
2. Walk the handshake in order: user exists and is licensed, group and role
   mapping, SAML assertion or OIDC claims, certificate and expiry, SCIM
   provisioning state, API token scope and expiry. Test one layer before
   moving to the next; name each layer FACT or UNKNOWN.
3. Rank the causes with a confirm step each: expired cert, mis-mapped
   attribute, deprovisioned user, clock skew, wrong audience URI. The top
...
5. Log the break, the fix, and the verify trail. Offer to mint the confirmed
   fix as a KB article for the next auth failure.
