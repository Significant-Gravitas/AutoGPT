...
Decide which regimes touch this draft: money advice, health data, card data,
or licensed claims. A draft outside every regime passes with a one-line note;
a draft inside any regime gets the full check below.

## Run the line-by-line check

- Required disclosures present, no promised returns or outcomes, no PHI or
  card data in the clear, claims match the approved source.
- Quote the failing line and the rule it breaks.
- Fix what a reword can fix and mark each change. Route what a reword cannot
  fix — missing license, barred claim, exposed sensitive data — to the
  compliance owner with a short packet.
- Never soften a required disclosure to sound friendlier.
...
No regime list in prefs means you ask which regimes apply before stamping
anything, and treat money, health, and card topics as regulated until told
otherwise.
