...
class TestRetryRateLimiting:
    ...
    @patch('backend.util.retry.get_notification_manager_client')
    def test_send_critical_retry_alert_handles_notification_failure(self, mock_get_client):
    ...
    def test_real_world_scenario_spend_credits_spam(self):
        ...
        exc = Exception("HTTP 500: Server error '500 Internal Server Error' for url 'http://autogpt-database-manager.prod-agpt.svc.cluster.local:8005/spend_credits'")
        
        # First 50 attempts reach threshold - should send alert
        with patch('backend.util.retry.get_notification_manager_client') as mock_get_client:
            mock_client = Mock()
            mock_get_client.return_value = mock_client
            
            _send_critical_retry_alert("_call_method_sync", 50, exc, "Service communication")
            assert mock_client.discord_system_alert.call_count == 1
