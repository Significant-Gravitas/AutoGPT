    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if request.include_graph_data and request.graph_id:
                try:
                    graph = await graph_db.get_graph(request.graph_id, user_id=request.user_id)
                    if graph:
                        nodes_data = []
                        for node in graph.nodes:
                            ...
                            if block:
                                node_data = {
                                    ...
                                    "block_type": block.block_type.value if hasattr(block, 'block_type') else None,
                                    "data": {
                                        k: v for k, v in (node.input_default or {}).items()
            ...
            async with session.post(
                ...
                logger.info(f"Successfully received response from Otto API for user {request.user_id}")
