...
class TestCacheDeletion:
    ...
    @pytest.mark.asyncio
    async def test_cache_invalidation_on_review(self):
        """Test that creating a review properly invalidates caches."""
        # This test simulates the behavior in create_review endpoint
        mock_agents_response = StoreAgentsResponse(
            agents=[],
            pagination=Pagination(
                total_items=0, total_pages=1, current_page=1, page_size=20
            ),
        )
        mock_details_response = StoreAgentDetails(
            store_listing_version_id="version1",
            slug="test-agent",
            agent_name="Test Agent",
            agent_video="https://example.com/video.mp4",
        ...
        with patch(
            "backend.server.v2.store.db.get_store_agents",
            new_callable=AsyncMock,
            return_value=mock_agents_response,
        ) as mock_agents_db, patch(
            "backend.server.v2.store.db.get_store_agent_details",
            new_callable=AsyncMock,
            return_value=mock_details_response,
        ) as mock_details_db:
            # Clear caches
            routes._get_cached_store_agents.cache_clear()
            routes._get_cached_agent_details.cache_clear()
            ...
            assert mock_agents_db.call_count == 1
            assert mock_details_db.call_count == 1

            # Simulate review creation - should clear specific caches
            routes._get_cached_agent_details.cache_delete(
                username="testuser", agent_name="testagent"
            )
            routes._get_cached_store_agents.cache_clear()
            ...
            assert mock_agents_db.call_count == 2  # Cache was cleared
            assert mock_details_db.call_count == 2  # Cache entry was deleted
