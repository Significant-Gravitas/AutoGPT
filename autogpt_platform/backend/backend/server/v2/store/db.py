...
async def get_store_agents(
    ...
    try:
        ...
        raise backend.util.exceptions.DatabaseError(
...
async def get_available_graph(store_listing_version_id: str) -> GraphMeta:
    try:
        ...
        raise backend.util.exceptions.DatabaseError("Failed to fetch agent") from e
...
async def get_store_creators(
    ...
    if search_query:
        ...
        if not sanitized_query or len(sanitized_query) > 100:  # Reasonable length limit
            raise backend.util.exceptions.DatabaseError("Invalid search query")
    ...
    try:
        ...
        if not isinstance(page, int) or page < 1:
            raise backend.util.exceptions.DatabaseError("Invalid page number")
        if not isinstance(page_size, int) or page_size < 1 or page_size > 100:
            raise backend.util.exceptions.DatabaseError("Invalid page size")
...
async def get_user_profile(
    ...
    try:
        ...
        raise backend.util.exceptions.DatabaseError("Failed to get user profile") from e
...
async def update_profile(
    ...
    try:
        ...
        if updated_profile is None:
            ...
            raise backend.util.exceptions.DatabaseError("Failed to update profile")
        ...
        raise backend.util.exceptions.DatabaseError("Failed to update profile") from e
...
async def get_my_agents(
    ...
    try:
        ...
        raise backend.util.exceptions.DatabaseError("Failed to fetch my agents") from e
