...
async def get_unprocessed_review_node_execution_entries(
    graph_exec_id: str, user_context
):
    """
    Get node execution entries for nodes with unprocessed completed reviews.

    This is used by the execution engine to get ready-to-execute node entries.

    Args:
        graph_exec_id: The graph execution ID to check
        user_context: User context for creating execution entries

    Returns:
        List of node execution entries ready for execution queue
    """
    ...
    # Convert node executions to execution entries
    node_entries = []
    for review in reviews:
        if review.NodeExecution:
            node_exec = NodeExecutionResult.from_db(review.NodeExecution)
            node_entry = node_exec.to_node_execution_entry(user_context)
