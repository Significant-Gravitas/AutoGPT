...
def _database(
    ...
    mocker.patch.object(
        history, "has_live_resource_access", AsyncMock(return_value=True)
    )
    return actions
...
async def test_archived_library_is_linkable_but_does_not_grant_run_access(mocker):
    ...
    assert item.agent_url is not None
...
async def test_public_nested_agent_can_be_named_without_library_entry(mocker):
    ...
    graph = _graph()
    graph.organizationId, graph.teamId = "publisher-org", "publisher-team"
    actions["StoreListingVersion"].find_many.return_value = [
        MagicMock(spec=StoreListingVersion, AgentGraph=graph)
    ]
    item = (await history.enrich_credit_history([_item()], "user"))[0]
    assert item.agent_name == "Original agent"
    assert item.execution_available is True
...
async def test_nested_runs_have_relationships_without_rolling_up_costs(mocker):
    ...
    assert child_item.parent_agent_url == parent_item.agent_url
    assert child_item.parent_execution_url == parent_item.execution_url
    ...
    assert related.agent_url is None
    assert related.execution_url is not None
    assert parse_qs(urlsplit(related.execution_url).query) == {
...
async def test_copilot_links_only_existing_owned_conversations(mocker):
    ...
    assert existing.conversation_url is not None
    assert parse_qs(urlsplit(existing.conversation_url).query) == {
        "organizationId": ["org"],
        "teamId": ["team"],
        "sessionId": ["chat"],
    }
    ...
    assert unavailable.conversation_url is None
...
@pytest.mark.asyncio
async def test_billing_permission_alone_never_enriches_resource_metadata(mocker):
    actions = _database(mocker)
    mocker.patch.object(
        history, "has_live_resource_access", AsyncMock(return_value=False)
    )
    original = _item()

    result = await history.enrich_credit_history([original], "user", "org")

    assert result == [original]
    assert result[0].amount == -12
    for action in actions.values():
        action.find_many.assert_not_awaited()


@pytest.mark.asyncio
async def test_same_graph_runs_keep_their_own_workspace_library_links(mocker):
    runs, libraries = [], []
    for team in ("team-a", "team-b"):
        runs.append(
            _execution(team).model_copy(
                update={"organizationId": "org", "teamId": team}
            )
        )
        library = _library(title=team, library_id=team)
        library.organizationId, library.teamId = "org", team
        libraries.append(library)
    _database(mocker, executions=runs, libraries=libraries, graphs=[_graph()])

    results = await history.enrich_credit_history(
        [_item(run.id) for run in runs], "user", "org"
    )

    assert [item.agent_name for item in results] == ["team-a", "team-b"]
    assert [item.library_agent_id for item in results] == ["team-a", "team-b"]
    for item in results:
        assert item.agent_url is not None and item.execution_url is not None
        scope = {"organizationId": ["org"], "teamId": [item.library_agent_id]}
        assert parse_qs(urlsplit(item.agent_url).query) == scope
        assert parse_qs(urlsplit(item.execution_url).query) == {
            **scope,
            "activeTab": ["runs"],
            "activeItem": [item.usage_execution_id],
        }
