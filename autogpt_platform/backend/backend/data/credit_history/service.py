...
async def get_credit_history(
    ...
    viewer_user_id selects the authenticated reader when billing is delegated.

    Org history is organization-wide, with no initiator or team filtering:
    authorized billing readers can read ledger run/graph/node IDs and block names.
    ...
    transactions = await enrich_credit_history(
        selected,
        user_id=viewer_user_id if viewer_user_id is not None else user_id,
        organization_id=(
