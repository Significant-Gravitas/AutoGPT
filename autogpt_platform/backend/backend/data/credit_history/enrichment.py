...
from backend.data.tenancy import get_user_team_ids, has_live_resource_access
from backend.util.tenancy_urls import builder_path, copilot_path, library_agent_path

_LibraryScope = tuple[str, str | None, str | None]
_LibraryVersionScope = tuple[str, int, str | None, str | None]
...
async def enrich_credit_history(
    ...
    if not any(item.usage_execution_id or item.usage_graph_id for item in items):
        return items
    if organization_id is not None and not await has_live_resource_access(
    ...
    scope = (
        {
            ...
            "OR": [{"teamId": None}, {"teamId": {"in": team_ids}}],
        }
        if organization_id is not None
        else {"userId": user_id, "organizationId": None}
    )
    execution_data, sessions = await asyncio.gather(
        _load_executions(items, user_id, cast(AgentGraphExecutionWhereInput, scope)),
        _load_sessions(items, user_id, cast(ChatSessionWhereInput, scope)),
    )
...
async def _load_sessions(
    items: list[CreditTransactionItem], user_id: str, scope: ChatSessionWhereInput
...
async def _load_agent_refs(
...
) -> tuple[dict[str, _AgentRef], dict[str, _AgentRef]]:
    graph_ids = sorted(
    ...
    libraries = await LibraryAgent.prisma().find_many(
        where={
            ...
            "isDeleted": False,
            "AND": [cast(LibraryAgentWhereInput, scope)],
        },
        include={"AgentGraph": True},
        order=[{"agentGraphVersion": "asc"}, {"id": "asc"}],
    )
    libraries = [library for library in libraries if library.AgentGraph]
    graphs = await _load_accessible_graphs(executions, scope)
    current: dict[_LibraryScope, LibraryAgent] = {
        (library.agentGraphId, library.organizationId, library.teamId): library
        for library in libraries
    }
    exact = {
        (
            library.agentGraphId,
            library.agentGraphVersion,
    ...
    fallback = {
        library.agentGraphId: _library_ref(library) for library in current.values()
    }
    return {
        execution.id: _execution_ref(execution, graphs, exact, current, user_id)
...
def _library_ref(library: LibraryAgent) -> _AgentRef:
    ...
    return _AgentRef(
        ...
        agent_url=library_agent_path(
            library.id, library.organizationId, library.teamId
        ),
    )
...
def _execution_ref(
    ...
    exact: dict[_LibraryVersionScope, LibraryAgent],
    current: dict[_LibraryScope, LibraryAgent],
    user_id: str,
) -> _AgentRef:
    key = (execution.agentGraphId, execution.agentGraphVersion)
    library = exact.get((*key, execution.organizationId, execution.teamId))
    graph = graphs.get(key)
    if graph is None and library and not library.isArchived:
        graph = library.AgentGraph
    destination = library or current.get(
        (execution.agentGraphId, execution.organizationId, execution.teamId)
    )
    name = (
    ...
    return _AgentRef(
        ...
        agent_url=(
            library_agent_path(
                destination.id, destination.organizationId, destination.teamId
            )
            if destination
            else None
        ),
        execution_url=(
            _execution_url(execution, destination, graph, user_id)
...
def _execution_url(
    ...
    if library:
        return library_agent_path(
            ...
            execution.teamId,
            active_tab="runs",
            active_item=execution.id,
        )
    if (graph.organizationId, graph.teamId) != (
        execution.organizationId,
        execution.teamId,
    ) or (graph.organizationId is None and graph.userId != user_id):
        return None
    return builder_path(
        execution.agentGraphId,
...
def _enrich_item(
    ...
    if execution_id.startswith(COPILOT_SESSION_PREFIX):
        ...
        item.conversation_url = (
            copilot_path(session.id, session.organizationId, session.teamId)
            if session
            else None
        )
        return item
    ...
    ref = (
        refs.get(execution.id) if execution else fallback.get(item.usage_graph_id or "")
    )
    if ref:
        item.agent_name = ref.name
        item.library_agent_id = ref.library_agent_id
        item.agent_url = ref.agent_url
        item.execution_url = ref.execution_url
...
def _add_related(
    ...
    if parent:
        ...
        item.parent_agent_url = related.agent_url
        item.parent_execution_url = related.execution_url
