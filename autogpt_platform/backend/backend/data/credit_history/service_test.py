...
@pytest.mark.asyncio
async def test_billing_delegate_enriches_as_viewer_without_changing_wallet(monkeypatch):
    query = AsyncMock(return_value=[history_row("execution:run")])
    enrichment = AsyncMock(side_effect=lambda items, **kwargs: items)
    monkeypatch.setattr(
        "backend.data.credit_history.service.query_raw_with_schema", query
    )
    ...
    page = await get_credit_history(
        "owner", viewer_user_id="viewer", viewer_organization_id="personal-org"
    )

    assert query.call_args.args[1] == "owner"
    assert page.transactions[0].user_id == "owner"
    assert page.transactions[0].amount == -50
    enrichment.assert_awaited_once_with(
        page.transactions, user_id="viewer", organization_id="personal-org"
    )
