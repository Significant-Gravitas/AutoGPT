...
### How roster and skill changes reach an environment

`poetry run seed-experts` (`backend/api/features/experts/deploy_seed.py`) loads both. It is what the dev and prod deploy's post-upgrade seed Job is meant to run (`templates/job-seed.yaml` in the `autogpt-server` chart of the infrastructure repo). It refuses to start while any migration is pending or failed, or while any workflow the roster preloads is missing from that environment's marketplace. In that case it names what is missing and writes nothing. It takes a Postgres advisory lock, then seeds the skills catalog and then the roster. It needs `SKILLS_CATALOG_TOKEN`. Running it twice changes nothing, so there is nothing to deploy by hand: merging a roster edit is enough. A deploy updates the template, its bundled-skill links and, on existing hires, only the presentation fields and untouched routines (see `seed.py`). An owner's name, avatar and edited routines are never touched. A catalog skill whose content changed gets a new listing version, and the old one is kept. Each installed copy records the version it came from (`installed_version`, which survives the owner's edits), and it is brought forward the next time its index is rebuilt (`copilot/tools/skill_updates.py`). An untouched copy takes the new version whole. An edited copy gets a three-way merge (`skill_merge.py`): the owner's changes stay, the rest updates, and where both changed the same lines the owner's version wins and the clash is recorded in the copy's `update_conflicts` metadata. Community listings never update installed copies.
