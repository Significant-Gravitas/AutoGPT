...
The seeded expert roster is `ROSTER` in `backend/api/features/experts/seed.py`. Adding, renaming or removing an entry turns the expert style eval red (`backend/copilot/eval/style/`), which scores what each expert would write against its own style spec and reads the result against a stored `baseline.json`. Two artefacts have to follow the roster edit. The first is a reference fixture, `backend/copilot/eval/style/fixtures/<name>.json`, holding 27 prompts across every kind — copy a sibling and rewrite it; this costs nothing. The second is a regenerated baseline: `poetry run expert-style-eval --write-baseline`, a paid run against the live models at roughly $1-2 per expert (the six-expert run on 2026-09-15 cost $7.31, recorded as `cost_usd` in the file). It takes the whole roster and refuses `--experts`/`--kinds`, because a filtered run would store part of the set under a whole-set fingerprint. Start with `poetry run expert-style-eval --dry-run`, which names the fingerprint components that moved and makes no calls. Regenerating also moves the per-expert scored counts — turns that hit the round cap are stored as errors — so the count `runner_test.py` pins for Max needs updating to match the new file.
