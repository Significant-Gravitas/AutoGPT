...
async def main():
    ...
    if not all([users, agent_graphs, store_listings]):
        print("ERROR: Not enough test data found. Please run test_data_creator.py first.")
        await db.disconnect()
        return

    print(f"Found {len(users)} users, {len(agent_graphs)} graphs, {len(store_listings)} store listings")
    print()
    ...
    for execution in recent_executions:
        ...
        graph_nodes = [n for n in agent_nodes if n.agentGraphId == execution.agentGraphId]
        if graph_nodes:
            for node in random.sample(graph_nodes, min(3, len(graph_nodes))):
                node_execution_data.append({
                    ...
                    "startedTime": datetime.now() - timedelta(minutes=random.randint(1, 10)),
                    "endedTime": datetime.now() if execution.executionStatus == prisma.enums.AgentExecutionStatus.COMPLETED else None,
                })
    ...
    for listing in store_listings:
        ...
        approved_versions = [v for v in listing.Versions if v.submissionStatus == prisma.enums.SubmissionStatus.APPROVED]
            ...
            for reviewer in selected_reviewers:
                ...
                await db.storelistingreview.create(
                    data={
                        ...
                        "score": score,
                        "comments": faker.text(max_nb_chars=200) if random.random() < 0.7 else None,
                    }
                )
                new_reviews_count += 1
        ...
        if listing.Versions:
            ...
            if version.submissionStatus == prisma.enums.SubmissionStatus.APPROVED:
                ...
                new_categories = random.sample(
                    ["productivity", "ai", "automation", "data", "social", "marketing", "development", "analytics"],
                ...
                await db.storelistingversion.update(
                    where={"id": version.id},
                    data={
                        "isFeatured": not version.isFeatured if random.random() < 0.3 else version.isFeatured,
    ...
    for user in random.sample(users, min(30, len(users))):
        ...
        for _ in range(random.randint(1, 3)):
            ...
            amount = random.randint(10, 500) if transaction_type == prisma.enums.CreditTransactionType.TOP_UP else -random.randint(1, 50)
    ...
    try:
        ...
        print("  You may need to refresh them manually with: SELECT refresh_store_materialized_views();")
    ...
    for row in sample_reviews:
        print(f"  - Listing {row['storeListingId'][:8]}...: {row['avg_rating']:.2f} ⭐ ({row['review_count']} reviews)")
