...
class TestDataCreator:
    ...
    async def update_test_profiles(self) -> List[Dict[str, Any]]:
        ...
        # DETERMINISTIC: Guarantee at least GUARANTEED_FEATURED_CREATORS featured
        ...
        print(f"🎯 Guaranteeing {num_featured} featured creators (min: {GUARANTEED_FEATURED_CREATORS})")
    ...
    async def create_test_store_submissions(self) -> List[Dict[str, Any]]:
        ...
        DETERMINISTIC: Guarantees minimum featured agents and top agents for E2E tests.
        """
        ...
        for user in self.users:
            ...
            for submission_index in range(4):
                ...
                try:
                    print(f"Creating store submission for user {user['id']} with graph {graph['id']}")
                    ...
                    if submission.store_listing_version_id:
                        # DETERMINISTIC LOGIC:
                        # - First GUARANTEED_TOP_AGENTS submissions are ALWAYS approved
                        # - First GUARANTEED_FEATURED_AGENTS of those are ALWAYS featured
                        # - Rest use random logic for variety
                        
                        should_approve = submission_counter <= GUARANTEED_TOP_AGENTS or random.random() < 0.4
                        ...
                        if should_approve:
                            try:
                                ...
                                # Mark as featured if we haven't hit the minimum yet
                                if should_feature:
                                    try:
                                        await prisma.storelistingversion.update(
                                        ...
                                        featured_count += 1
                                        print(f"🌟 Marked agent as FEATURED ({featured_count}/{GUARANTEED_FEATURED_AGENTS}): {submission.name}")
                                    except Exception as e:
                                        print(f"Warning: Could not mark submission as featured: {e}")
                                elif random.random() < 0.2:
                                    # Additional random featured agents beyond minimum
                                    try:
                                        await prisma.storelistingversion.update(
                                            where={"id": submission.store_listing_version_id},
                                            data={"isFeatured": True},
                                        ...
                                        featured_count += 1
                                        print(f"🌟 Marked agent as FEATURED (bonus): {submission.name}")
                                    except Exception as e:
                                        print(f"Warning: Could not mark submission as featured: {e}")
                            ...
                            # 50% of non-approved get rejected, rest stay pending
                            try:
                                reviewer_id = random.choice(self.users)["id"]
                                ...
                                print(f"Warning: Could not reject submission {submission.name}: {e}")
                        else:
    ...
    async def create_all_test_data(self):
        ...
        print("\n🎯 Deterministic Guarantees (for E2E test reliability):")
        ...
        print(f"   • Library agents per user: >= {MIN_AGENTS_PER_USER} (for pagination)")
