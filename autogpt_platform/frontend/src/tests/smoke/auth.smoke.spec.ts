...
test("@smoke auth happy path: user can sign up with a fresh account", async ({
  page,
}) => {
...
test("@smoke auth happy path: seeded user can log in", async ({ page }) => {
  test.setTimeout(90000);
...
test("@smoke auth happy path: user can complete onboarding and land in the app", async ({
  page,
}) => {
  test.setTimeout(90000);
