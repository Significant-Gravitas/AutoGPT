...
describe("Marketplace ExpertsSection", () => {
  ...
  test("refreshes hired badges when the selected workspace changes", async () => {
    const scopes: Array<string | null> = [];
    useOrgTeamStore.setState({
      activeOrgID: "personal-org",
      activeTeamID: "personal-team",
    });
    server.use(
      getListExpertTemplatesMockHandler([mariaTemplate]),
      getListExpertsMockHandler(({ request }) => {
        const org = request.headers.get("X-Org-Id");
        scopes.push(org);
        return org === "personal-org" ? [hiredMaria] : [];
      }),
    );
    render(<MainMarkeplacePage />);
    ...
    await waitFor(() => expect(scopes).toContain("shared-org"));
    expect(screen.queryByText("Hired")).toBeNull();
    expect(scopes).toContain("personal-org");
  });
