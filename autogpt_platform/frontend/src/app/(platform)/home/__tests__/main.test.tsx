...
test("switching tenants replaces Home data and sends the new tenant headers", async () => {
  ...
  server.use(
    http.get(/\/api\/proxy\/api\/home(?:\?.*)?$/, ({ request }) => {
      const org = request.headers.get("X-Org-Id");
      scopes.push(org);
      return HttpResponse.json({
        ...dashboard,
        attention: [{ ...dashboard.attention[0], title: `${org} attention` }],
      });
    }),
  );
  render(<HomePage />);
  expect(await screen.findByText("org-one attention")).toBeDefined();
  act(() =>
    useOrgTeamStore.setState({
      activeOrgID: "org-two",
  ...
  expect(await screen.findByText("org-two attention")).toBeDefined();
  expect(screen.queryByText("org-one attention")).toBeNull();
  expect(scopes).toEqual(["org-one", "org-two"]);
});
