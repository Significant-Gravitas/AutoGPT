...
function StatusBadge({ status }: { status: HomeAgentStatus["status"] }) {
  const config = {
    working: { label: "Working", icon: Loading03Icon },
    ready: { label: "Ready", icon: CheckmarkCircle02Icon },
    needs_setup: { label: "Setup", icon: Settings01Icon },
    paused: { label: "Paused", icon: PauseIcon },
    failed: { label: "Failed", icon: AlertCircleIcon },
  }[status];
  ...
  return (
    <span
      className={cn(
        ...
        status === "needs_setup" && "bg-amber-50 text-amber-700",
        status === "paused" && "bg-zinc-100 text-zinc-600",
