...
import { CopilotTenantScopeProvider } from "@/app/(platform)/copilot/CopilotTenantScopeContext";
import {
  getCopilotHref,
...
export function ExpertChatDrawer({
  ...
  const copilotHref = chat.sessionId
    ? getCopilotHref(
        chat.sessionId,
        chat.scope.organizationId,
        chat.scope.teamId,
      )
    : target?.expertId
      ? `${getCopilotExpertHref(target.expertId, chat.scope.organizationId, chat.scope.teamId)}&new=1`
      : getCopilotStartHref(chat.scope.organizationId, chat.scope.teamId);
...
function ChatPanelBody({ target, identity, chat }: BodyProps) {
  ...
  return (
    <CopilotTenantScopeProvider {...chat.scope}>
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {isResolvingSession ? (
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              <ChatMessagesContainer
                ...
                isLoading={false}
                sessionID={sessionId}
                organizationId={chat.scope.organizationId}
                teamId={chat.scope.teamId}
          ...
          <div className="shrink-0 px-3 pb-5 pt-2">
            <ChatInput
              ...
              variant="compact"
              onSend={onSend}
              disabled={isResolvingSession || isCreating || !chat.isScopeReady}
