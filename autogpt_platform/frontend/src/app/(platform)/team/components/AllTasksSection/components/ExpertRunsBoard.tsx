...
export function ExpertRunsBoard({ experts, enabled }: Props) {
        ...
        <ul className="flex flex-col gap-3">
          {tasks.map((task) => (
            <li key={task.run.execution_id}>
              <RunRow task={task} onOpen={() => setActiveTask(task)} />
            </li>
          ))}
        </ul>
      )}
...
function RunRow({ task, onOpen }: { task: ExpertTask; onOpen: () => void }) {
