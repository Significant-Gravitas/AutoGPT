...
describe("ArtifactsPage - folders", () => {
  ...
  test.each(["create", "cancel"])(
    "keeps the page interactive after %s closes the new-folder dialog",
    async (action) => {
      useStorageHandler();
      server.use(
        getListWorkspaceFilesMockHandler({
        ...
        getListWorkspaceFoldersMockHandler({ folders: [makeFolder()] }),
        http.post(`${PROXY}/folders`, () =>
          HttpResponse.json(makeFolder({ name: "Invoices" }), { status: 201 }),
        ),
      );
      const user = userEvent.setup();
      render(<ArtifactsPage />);
      await screen.findByTestId("workspace-folder");
      fireEvent.pointerDown(screen.getByTestId("artifacts-new-menu"), {
        button: 0,
      });
      const createFolder = await screen.findByTestId("create-folder-button");
      ...
      await user.click(screen.getByTestId("workspace-folder"));
      expect(await screen.findByTestId("folder-breadcrumb")).toBeDefined();
    },
  );
