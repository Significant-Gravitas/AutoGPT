...
describe("AvatarPage", () => {
  test("starts from the default expert and updates the stage per pick", async () => {
    ...
    await user.click(pickerRadio("Shape", "Wide"));
    await user.click(pickerRadio("Colour", "Mint"));
    await user.click(pickerRadio("Accessory", "Headset"));

    expect(stageAvatar().getAttribute("data-avatar")).toBe("wide.mint.headset");
    expect(pickerRadio("Shape", "Wide").getAttribute("aria-checked")).toBe(
      "true",
    );
