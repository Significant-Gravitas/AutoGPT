  ...
  return (
    <div className="w-full">
      {videoId ? (
        ...
        <video controls width="100%" height="315" className="rounded-xlarge">
...
}> = ({ value, truncateLengthLimit, isViewMoreData }) => {
  const text =
    typeof value === "object" ? JSON.stringify(value, null, 2) : String(value);
  return truncateLengthLimit && text.length > truncateLengthLimit ? (
    <Text
      variant={isViewMoreData ? "body" : "small"}
...
export const ContentRenderer: React.FC<{
  ...
  isViewMoreData?: boolean;
}> = ({ value, truncateLongData, isViewMoreData }) => {
  ...
  return (
    <TextRenderer
      value={value}
      truncateLengthLimit={
        isViewMoreData ? 300 : truncateLongData ? 100 : undefined
      }
      isViewMoreData={isViewMoreData}
    />
  );
};
