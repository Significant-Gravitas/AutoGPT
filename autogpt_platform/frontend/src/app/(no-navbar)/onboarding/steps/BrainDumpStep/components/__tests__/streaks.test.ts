...
describe("streak geometry", () => {
  it("fits a finite orbit around the avatar's disc", () => {
    const field = createStreakField(AUTOPILOT_AURA_COLORS, 160);
    expect(field.box).toBe(240);
    expect(field.profile).toHaveLength(72);
    expect(field.profile.every((radius) => radius === 80)).toBe(true);
