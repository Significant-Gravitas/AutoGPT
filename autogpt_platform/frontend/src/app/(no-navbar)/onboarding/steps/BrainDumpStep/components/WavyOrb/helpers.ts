...
export function follow(current: number, target: number, delta: number) {
...
interface RendererOptions {
  ...
  getSettings: () => WavyOrbSettings;
  prefersReducedMotion: boolean;
}

export interface WavyOrbRenderer {
  draw: (now: number) => void;
  dispose: () => void;
}

export function createWavyOrbRenderer({
  canvas,
  audioStream,
  getSettings,
  prefersReducedMotion,
}: RendererOptions): WavyOrbRenderer | null {
