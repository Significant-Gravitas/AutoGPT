...
test("approving a later group sends that group's tenant headers", async () => {
  let capturedHeaders: Headers | undefined;
  let capturedBody: ReviewRequest | undefined;
  server.use(
    getPostV2ProcessReviewActionMockHandler200(async ({ request }) => {
      capturedHeaders = request.headers;
      capturedBody = (await request.json()) as ReviewRequest;
      return { approved_count: 1, rejected_count: 0, failed_count: 0 };
    }),
  );
  render(
  ...
  expect(capturedBody?.reviews.map((review) => review.node_exec_id)).toEqual([
    "later",
  ]);
  expect(capturedHeaders?.get("X-Org-Id")).toBe("org-second");
  expect(capturedHeaders?.get("X-Team-Id")).toBe("team-second");
});
