...
describe("Dialog", () => {
  ...
  test("associates an sr-only description echoing the title", () => {
    renderDialog({ title: "My Title" });
    ...
    expect(description?.textContent).toBe("My Title");
    expect(description?.classList.contains("sr-only")).toBe(true);
  });

  test("associates a default sr-only description when title is missing", () => {
    renderDialog({});
