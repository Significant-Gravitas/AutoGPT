...
export function useInstallWorkflowPicker({
  ...
  const expertsQuery = useListExperts({
    ...
    request: getTenantRequestInit(organizationId, teamId, isLoaded),
  });
  ...
  const libraryResultsQuery = useGetV2ListLibraryAgentsInfinite(libraryParams, {
    query: {
      queryKey: getTeamScopedQueryKey(
        ...
        targetExpert ? (targetExpert.organization_id ?? null) : organizationId,
        targetExpert ? (targetExpert.team_id ?? null) : teamId,
      ),
      initialPageParam: 1,
      getNextPageParam: (last) =>
        ...
        last.data.pagination.current_page < last.data.pagination.total_pages
          ...
          : undefined,
      enabled: isPickingWorkflow && source === "library" && isLoaded,
    },
