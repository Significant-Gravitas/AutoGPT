...
class AutoGpt():
    def __init__(self, run_main_loop=True):
      ...
      self.memory = get_memory(cfg, init=True)
    ...
    def run_main_loop(self):
        ...
        while True:
            ...
            self.print_assistant_thoughts(assistant_reply)
            ...
            if result is not None:
                ...
                self.print_to_console("SYSTEM: ", Fore.YELLOW, result)
            else:
                self.full_message_history.append(
                    chat.create_chat_message(
                        "system", "Unable to execute command"))
                self.print_to_console("SYSTEM: ", Fore.YELLOW, "Unable to execute command")
    ...
    def print_assistant_thoughts(self, assistant_reply):
        ...
        try:
            ...
            self.print_to_console(f"{ai_name.upper()} THOUGHTS:", Fore.YELLOW, assistant_thoughts_text)
            self.print_to_console("REASONING:", Fore.YELLOW, assistant_thoughts_reasoning)

            if assistant_thoughts_plan:
                self.print_to_console("PLAN:", Fore.YELLOW, "")
                ...
                for line in lines:
                    line = line.lstrip("- ")
                    self.print_to_console("- ", Fore.GREEN, line.strip())

            self.print_to_console("CRITICISM:", Fore.YELLOW, assistant_thoughts_criticism)
    ...
    def parse_arguments(self):
        ...
        if args.continuous:
            self.print_to_console("Continuous Mode: ", Fore.RED, "ENABLED")
            self.print_to_console(
                "WARNING: ",
                Fore.RED,
        ...
        if args.speak:
            self.print_to_console("Speak Mode: ", Fore.GREEN, "ENABLED")
            cfg.set_speak_mode(True)

        if args.gpt3only:
            self.print_to_console("GPT3.5 Only Mode: ", Fore.GREEN, "ENABLED")
            cfg.set_smart_llm_model(cfg.fast_llm_model)

        if args.gpt4only:
            self.print_to_console("GPT4 Only Mode: ", Fore.GREEN, "ENABLED")
            cfg.set_fast_llm_model(cfg.smart_llm_model)

        if args.debug:
            self.print_to_console("Debug Mode: ", Fore.GREEN, "ENABLED")
