...
def mock_html_file():
    html = BeautifulSoup(
        f"<html><head><title>This is a test</title></head>"
        f"<body><p>{plain_text_str}</p></body></html>",
        "html.parser",
    )
...
def mock_latex_file():
    with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".tex") as f:
        latex_str = rf"\documentclass{{article}}\begin{{document}}\
            {plain_text_str}\end{{document}}"
