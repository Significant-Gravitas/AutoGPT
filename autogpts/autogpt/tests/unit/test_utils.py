...
def valid_json_response() -> dict:
    return {
        "thoughts": {
            "text": (
                "My task is complete. I will use the 'task_complete' "
                "command to shut down."
            ),
            "reasoning": (
                "I will use the 'task_complete' command because it allows me "
                "to shut down and signal that my task is complete."
            ),
            "plan": (
                "I will use the 'task_complete' command with the reason "
                "'Task complete: "
                "retrieved Tesla's revenue in 2022.' to shut down."
            ),
            "criticism": (
                "I need to ensure that I have completed all necessary tasks "
...
def invalid_json_response() -> dict:
    return {
        "thoughts": {
            "text": (
                "My task is complete. I will use the 'task_complete' command "
                "to shut down."
            ),
            "reasoning": (
                "I will use the 'task_complete' command because it allows me "
...
def test_get_bulletin_from_web_success(mock_get):
    ...
    mock_get.assert_called_with(
        (
            ...
            "Significant-Gravitas/AutoGPT/master/autogpts/autogpt/BULLETIN.md"
        )
    )
...
def test_get_current_git_branch():
    ...
    # Assuming that the branch name will be non-empty if the function
