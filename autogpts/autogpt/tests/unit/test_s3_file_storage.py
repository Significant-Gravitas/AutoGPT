...
@pytest.fixture
def s3_workspace_uninitialized(s3_bucket_name: str) -> S3FileStorage:
    ...
    workspace = S3FileStorage(storage_config)
    yield workspace  # type: ignore
    del os.environ["STORAGE_BUCKET"]


def test_initialize(s3_bucket_name: str, s3_workspace_uninitialized: S3FileStorage):
...
@pytest.fixture
def s3_workspace(s3_workspace_uninitialized: S3FileStorage) -> S3FileStorage:
...
async def s3_workspace_with_files(s3_workspace: S3FileStorage) -> S3FileStorage:
...
@pytest.mark.asyncio
async def test_read_file(s3_workspace_with_files: S3FileStorage):
    for file_name, file_content in TEST_FILES:
...
def test_list_files(s3_workspace_with_files: S3FileStorage):
    ...
    ) == s3_workspace_with_files.list_files()
    assert len(files) > 0
...
@pytest.mark.asyncio
async def test_write_read_file(s3_workspace: S3FileStorage):
    await s3_workspace.write_file("test_file", "test_content")
    assert s3_workspace.read_file("test_file") == "test_content"


@pytest.mark.asyncio
async def test_overwrite_file(s3_workspace_with_files: S3FileStorage):
    for file_name, _ in TEST_FILES:
...
def test_delete_file(s3_workspace_with_files: S3FileStorage):
