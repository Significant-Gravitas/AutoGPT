...
class ForgeAgent(Agent):
    ...
    async def execute_step(self, task_id: str, step_request: StepRequestBody, is_retry: bool = False) -> Step:
        ...
        if not steps:
            ability = self.select_ability(task)
    ...
    async def execute_step_planning_mode(self, task_id: str, step_request: StepRequestBody, is_retry: bool = False) -> Step:
        task = await self.db.get_task(task_id)
        ...
        if not steps:
            if planning_mode:
                ...
                ability = self.select_ability(step_request.input)

        if planning_mode and not next_steps:
    ...
    async def select_ability(self, task: Task):
        ...
        try:
            ...
            ability_names = [a.name for a in self.abilities.list_abilities().values()]
            if isinstance(ability, dict) and ability["name"] in ability_names:
                if ability != ability:
                    LOG.info(f"Update ability {ability['name']} with {ability['name']}")
                ability = ability
            else:
