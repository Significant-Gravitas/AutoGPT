...
async def create_test_graph() -> graph.Graph:
    ...
    links = [
        graph.Link(nodes[0].id, nodes[1].id, "output", "text"),
    ]
...
async def execute_agent(test_manager: ExecutionManager, test_graph: graph.Graph):
    input_data = {
        "task": "Make calculations you receive on input and output the result."
    ...
    executions = await agent_server.get_run_execution_results(test_graph.id, graph_exec_id)

    # Executing AgentBlock
    exec = executions[0]
    assert exec.status == execution.ExecutionStatus.COMPLETED
    assert exec.graph_exec_id == graph_exec_id
    assert "8" in exec.output_data["output"]
