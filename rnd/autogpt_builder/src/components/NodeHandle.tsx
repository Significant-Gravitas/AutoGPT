...
const NodeHandle: FC<HandleProps> = ({
  ...
  const typeClass = `text-sm ${getTypeTextColor(schema.type)} ${
  ...
  const dot = (
    <div
      className={`w-4 h-4 m-1 ${
        isConnected ? getTypeBgColor(schema.type) : "bg-gray-600"
      } rounded-full transition-colors duration-100 group-hover:bg-gray-300`}
    />
  );
