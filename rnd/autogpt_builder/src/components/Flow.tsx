        ...
        <div className={`sidebar dark-theme ${isOpen ? 'open' : ''}`}>
            ...
            {filteredNodes.map((node) => (
                <div key={node.id} className="sidebarNodeRowStyle dark-theme">
                    <span>
                        {beautifyString(node.name).replace(/Block$/, '')}
                    </span>
                    <Button onClick={() => addNode(node.id, node.name)}>
                        Add
                    </Button>
                </div>
            ))}
...
const FlowEditor: React.FC<{
                ...
                const newNode: Node<CustomNodeData> = {
                    ...
                    data: {
                        ...
                        connections: graph.links
                            .filter((l) =>
                                [l.source_id, l.sink_id].includes(node.id)
                            )
                            .map((link) => ({
    ...
    const prepareNodeInputData = (
        ...
        if (!blockSchema) {
            console.error(
                `Schema not found for block ID: ${node.data.block_id}`
            )
            return {}
        }
                ...
                if (event.key === 'v' || event.key === 'V') {
                    ...
                    if (copiedNodes.length > 0) {
                        ...
                        const newEdges = copiedEdges.map((edge) => {
                            const newSourceId =
                                newNodes.find(
                                    (n) => n.data.title === edge.source
                                )?.id || edge.source
                            const newTargetId =
                                newNodes.find(
                                    (n) => n.data.title === edge.target
                                )?.id || edge.target
                            return {
                                ...edge,
            ...
            >
                <div style={{ position: 'absolute', right: 10, zIndex: 4 }}>
                    ...
                    >
                        ...
                        <Button
                            onClick={() => saveAgent(savedAgent?.is_template)}
                        >
                            Save{' '}
                            {savedAgent?.is_template ? 'Template' : 'Agent'}
                        </Button>
                        {!savedAgent?.is_template && (
                            <Button onClick={runAgent}>Save & Run Agent</Button>
                        )}
                        {!savedAgent && (
                            <Button onClick={() => saveAgent(true)}>
                                Save as Template
                            </Button>
                        )}
                    </div>
