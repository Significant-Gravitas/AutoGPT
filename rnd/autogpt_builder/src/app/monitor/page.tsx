            ...
            runIDs.map((runID) => {
                ...
                api.getGraphExecutionInfo(flowID, runID).then((execInfo) =>
                    setFlowRuns((flowRuns) => {
                        ...
                        const flowRunIndex = flowRuns.findIndex(
                            (fr) => fr.id == runID
                        )
                        const flowRun =
                            flowRunFromNodeExecutionResults(execInfo)
                        if (flowRunIndex > -1) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 lg:grid-cols-4 xl:grid-cols-10 gap-4">
            ...
            {(selectedRun && (
                <FlowRunInfo
                    flow={
                        selectedFlow ||
                        flows.find((f) => f.id == selectedRun.graphID)!
                    }
                    flowRun={selectedRun}
                    className={column3}
                />
...
function flowRunFromNodeExecutionResults(
    ...
    const endTime = ['success', 'failed'].includes(status)
        ? Math.max(
              ...nodeExecutionResults.map(
                  (ner) => ner.end_time?.getTime() || 0
              ),
              startTime
          )
        : now
                ...
                <TableBody>
                    {runs.map((run) => (
                        <TableRow
                            ...
                            data-state={
                                selectedRun?.id == run.id ? 'selected' : null
                            }
                        >
                            <TableCell>
                            ...
                            <TableCell>
                                {moment(run.startTime).format('HH:mm')}
                            </TableCell>
                            <TableCell>
...
> = ({ flow, flowRuns, flowVersion, ...props }) => {
    ...
    const selectedFlowVersion: Graph | undefined = flowVersions?.find(
        (v) =>
            v.version ==
            (selectedVersion == 'all' ? flow.version : selectedVersion)
    )

    useEffect(() => {
        api.getGraphAllVersions(flow.id).then((result) =>
            setFlowVersions(result)
        )
    }, [flow.id])
        ...
        <Card {...props}>
            <CardHeader className="flex-row justify-between space-y-0 space-x-3">
                <div>
                    <CardTitle>
                        {flow.name}{' '}
                        <span className="font-light">v{flow.version}</span>
                    </CardTitle>
                    <p className="mt-2">
                ...
                <FlowRunsStats
                    ...
                    flowRuns={flowRuns.filter(
                        (r) =>
                            r.graphID == flow.id &&
                            (selectedVersion == 'all' ||
                                r.graphVersion == selectedVersion)
                    )}
                />
            </CardContent>
        </Card>
            ...
            <div>
                ...
                <p>
                    ...
                    {filteredFlowRuns.reduce(
                        (total, run) => total + run.totalRunTime,
                        0
                    )}{' '}
                    seconds
                </p>
