...
export default class AutoGPTServerAPI {
    ...
    async createTemplate(templateCreateBody: GraphCreatable): Promise<Graph> {
        const requestBody: GraphCreateRequestBody = {
            graph: templateCreateBody,
        }
    ...
    private async _request(
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `HTTP error ${response.status}! ${response_data.detail}`
            )
        }
