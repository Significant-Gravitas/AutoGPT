...
from autogpt.commands.openapi import load_openapi_config, OPENAPI_COMMANDS_REGISTRY, AutoGPTOpenAPIConfig, \
    get_openapi_specs_and_manifests, initialize_openapi_plugins
...
def test_get_openapi_manifests_and_specs_from_manifest(weathergpt_openai_config):
    specs_manifests = get_openapi_specs_and_manifests(weathergpt_openai_config, plugins_dir=PLUGINS_TEST_DIR)
...
def test_get_openapi_manifests_and_specs_from_openapi_spec(weatherapi_config):
    specs_manifests = get_openapi_specs_and_manifests(weatherapi_config, plugins_dir=PLUGINS_TEST_DIR)
    assert specs_manifests['weathergpt']['manifest']
    assert specs_manifests['weathergpt']['openapi_spec']
...
def test_initialize_openapi_plugins(weathergpt_openai_config):
    specs_manifests = get_openapi_specs_and_manifests(weathergpt_openai_config, plugins_dir=PLUGINS_TEST_DIR)
    initalized_openapi_plugins = initialize_openapi_plugins(specs_manifests, weathergpt_openai_config,
                                                            PLUGINS_TEST_DIR)
