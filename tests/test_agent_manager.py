...
def test_create_agent(request, mocker, agent_manager, task, prompt, model):
...
def test_message_agent(request, mocker, agent_manager, task, prompt, model):
...
def test_list_agents(request, mocker, agent_manager, task, prompt, model):
    key, _ = agent_manager.create_agent(task, prompt, model)
...
def test_delete_agent(request, mocker, agent_manager, task, prompt, model):
    key, _ = agent_manager.create_agent(task, prompt, model)
