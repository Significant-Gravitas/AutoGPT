# sourcery skip: snake-case-functions
"""Tests for RedisMemory class"""
import pytest

from autogpt.config import Config
from autogpt.memory.context import MemoryItem, RedisMemory
from tests.utils import requires_api_key


@pytest.fixture(autouse=True)
def cleanup_sut_singleton():
    if RedisMemory in RedisMemory._instances:
        del RedisMemory._instances[RedisMemory]


def test_redis_memory_add(config: Config, memory_item: MemoryItem):
    index = RedisMemory(config)
    index.add(memory_item)
    assert memory_item in index


def test_redis_memory_clear(config: Config, memory_item: MemoryItem):
    index = RedisMemory(config)
    assert len(index) == 0

    index.add(memory_item)
    assert len(index) == 1, "Cannot test clear() because add() fails"

    index.clear()
    assert len(index) == 0


def test_redis_memory_get(config: Config, memory_item: MemoryItem, mock_get_embedding):
    index = RedisMemory(config)
    assert (
        index.get("test") == None
    ), "Cannot test get() because initial index is not empty"

    index.add(memory_item)
    retrieved = index.get("test")
    assert retrieved is not None
    assert retrieved.memory_item == memory_item


@pytest.mark.vcr
@requires_api_key("OPENAI_API_KEY")
def test_redis_memory_get_relevant(config: Config) -> None:
    index = RedisMemory(config)
    mem1 = MemoryItem.from_text_file("Sample text", "sample.txt")
    mem2 = MemoryItem.from_text_file("Grocery list:\n- Pancake mix", "groceries.txt")
    mem3 = MemoryItem.from_text_file("What is your favorite color?", "color.txt")
    lipsum = "Lorem ipsum dolor sit amet"
    mem4 = MemoryItem.from_text_file(" ".join([lipsum] * 100), "lipsum.txt")
    index.add(mem1)
    index.add(mem2)
    index.add(mem3)
    index.add(mem4)

    assert index.get_relevant(mem1.raw_content, 1)[0].memory_item == mem1
    assert index.get_relevant(mem2.raw_content, 1)[0].memory_item == mem2
    assert index.get_relevant(mem3.raw_content, 1)[0].memory_item == mem3
    assert [mr.memory_item for mr in index.get_relevant(lipsum, 2)] == [mem4, mem1]


def test_redis_memory_get_stats(config: Config, memory_item: MemoryItem) -> None:
    index = RedisMemory(config)
    index.add(memory_item)
    n_memories, n_chunks = index.get_stats()
    assert n_memories == 1
    assert n_chunks == 1
