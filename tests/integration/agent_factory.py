...
from autogpt.memory import NoMemory, get_memory, LocalCache
...
@pytest.fixture
def memory_management_agent(agent_test_config, memory_none: LocalCache, workspace: Workspace):
    command_registry = CommandRegistry()
    command_registry.import_commands("autogpt.commands.file_operations")
    command_registry.import_commands("autogpt.app")
    ...
    ai_config = AIConfig(
        ...
        ai_role="an AI designed to read the instructions.txt file using the read_file method and follow the instructions in the file.",
        ai_goals=[
