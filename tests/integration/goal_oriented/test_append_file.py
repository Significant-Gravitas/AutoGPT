        ...
        with my_vcr.use_cassette(
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                try:
                    ...
                    assert False, "The process took longer than 90 seconds to complete."
    # catch system exit exceptions
    except SystemExit:  # the agent returns an exception when it shuts down
        content = read_file(file_name)
        assert content == "Hello World, AGI is coming", f"Expected 'Hello World, AGI is coming', got {content}"
...
def create_writer_agent(workspace):
    ...
    ai_config = AIConfig(
        ...
        ai_role="an AI designed to use the write_to_file command to write 'Hello World' into a file named \"hello_world.txt\" and then use the append_to_file command to append the text 'AGI is coming' to \"hello_world.txt\" and use the task_complete command to complete the task.",
        ai_goals=[
            "Use the write_to_file command to write 'Hello World' into a file named \"hello_world.txt\".",
            "Use the append_to_file command to append the text ', AGI is coming' to \"hello_world.txt\".",
