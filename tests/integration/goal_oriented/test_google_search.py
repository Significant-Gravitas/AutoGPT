...
def test_write_file(workspace) -> None:
    ...
    file_name = str(workspace.get_path("google_IP.txt"))
    agent = create_writer_agent(workspace)
    try:
        with my_vcr.use_cassette(
            "google_search.vcr.yml",
            filter_headers=[
                "authorization",
...
def create_writer_agent(workspace):
    ...
    ai_config = AIConfig(
        ai_name="google_IP-GPT",
        ai_role="an AI designed to use the google and write_to_file command to write the user's IP address into a file named \"google_search_IP.txt\" and then use the task_complete command to complete the task.",
        ai_goals=[
            "Use the google command to query 'What's my ip' and return the users IP address",
            "Use the write_to_file command to write user's IP address into a file named \"google_search_IP.txt\".",
