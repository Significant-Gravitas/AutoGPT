...
@pytest.mark.asyncio
async def test_default_post_prossessing_retry(example_tool: Tool, default_task: Task, mocker):
    # Mock dependencies
    ...
    mock_search_result.parsed_result = [ { "command_name" : example_tool.name,
        "command_args": {'text_output': 'Processed output', 'text_output_as_uml': 'UML output'},
    ...
    assert default_task.task_text_output == 'Processed output', "The task text output should be updated with the processed output"
    assert default_task.task_text_output_as_uml == 'UML output', "The task UML output should be updated accordingly"
...
async def test_tool_execution_summarry(example_tool: Tool, default_task: Task, mocker):
    ...
    mock_strategy_result.parsed_result = [{'command_args': {'text_output': 'Processed output', 'text_output_as_uml': 'UML output'}}]
    mocker.patch.object(default_task.agent, 'execute_strategy', new_callable=AsyncMock(mock_strategy_result))
