...
def main():
    ...
    # Create parent span
    wandb_span = trace_tree.Span(name="Auto-GPT")
    wandb_span.span_kind = trace_tree.SpanKind.AGENT
...
def print_wandb_finish_message(run_url: str) -> None:
    wandb.termlog(f"View W&B Run at {run_url}")

def flush_wandb(wandb_span):
    run = wandb.init(entity="prompt-eng", project="auto-gpt", settings = {"silent": True})
    run.config = vars(cfg)
    run_url = run.get_url()
    run.log({"trace": trace_tree.WBTraceTree(wandb_span)})
    run.finish()
    wandb.termlog(f"View your Weights & Biases Run at {run_url}")
...
class Agent:
    ...
    def __init__(
        ...
        self.wandb_span = wandb_span
        self.wandb_span_inputs = {"prompt": prompt, 
                                    "full_message_history": full_message_history,
                                    "user_input": user_input}

        # self.agent_span = trace_tree.Span(name=f"Auto-GPT Agent")
    ...
    def start_interaction_loop(self):
        ...
        while True:
            ...
            if (
                ...
                # WANDB CODE
                self.wandb_span.add_named_result()
                self.wandb_span.add_named_result(inputs = {"input": self.wandb_span_inputs},
                                                 output = {"response": outputs})
                flush_wandb(self.wandb_span)
                break
            ...
            with Spinner("Thinking... "):
                ...
                )  # TODO: This hardc odes the model to use GPT3.5. Make this an argument
            ...
            try:
                ...
                # WANDB CODE
                wandb_ai_inputs = {"prompt":self.prompt,
                                "user_input": self.user_input,
                                "full_message_history":self.full_message_history
                        #  "full_message_history":self.full_message_history, 
                        #  "memory":self.memory, 
                        #  "fast_token_limit":cfg.fast_token_limit
                }
                if loop_count == 1:
                    wandb_span_input = wandb_ai_inputs

                wandb_ai_outputs = {"assistant_reply": assistant_reply,
                        "command_name": command_name,
                        "arguments": arguments
                }
                inputs = {"input": wandb_ai_inputs}
                outputs = {"output": wandb_ai_outputs}

                wandb_child_span = trace_tree.Span(name=f"{command_name}")
                wandb_child_span.span_kind = trace_tree.SpanKind.TOOL
                wandb_child_span.add_named_result(inputs, outputs)

                self.wandb_span.add_child_span(wandb_child_span)

            except Exception as e:
                self.wandb_span.add_named_result({"input": wandb_span_input}, {"response": e})
                flush_wandb(wandb_span)
                logger.error("Error: \n", str(e))
            ...
            if not cfg.continuous_mode and self.next_action_count == 0:
                ...
                if self.user_input == "GENERATE NEXT COMMAND JSON":
                    ...
                    # WANDB CODE
                    self.wandb_span.add_named_result({"input": wandb_span_input}, {"response": outputs})
                    flush_wandb(self.wandb_span)
