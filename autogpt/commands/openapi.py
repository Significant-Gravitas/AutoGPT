...
def load_openapi_config(cfg: Config, openapi_registry_path: str = OPENAPI_COMMANDS_REGISTRY) -> dict[OpenAPIConfig]:
    """
    ...
    for api_name, api_config in openapi_registry.items():
        if api_name in cfg.openapi_apis and api_config["enabled"]:
            enabled_openapi_apis[api_name] = OpenAPIConfig(**api_config)
...
def get_openapi_spec(openapi_spec_url: str, openapi_plugin_client_dir: str) -> dict:
    """
...
def generate_openai_manifest_from_openapi_spec(openapi_spec: dict) -> dict:
    """
...
def get_openai_manifest(openapi_manifest_url: str, openapi_spec: Optional[dict],
                        openapi_plugin_client_dir: str) -> dict:
    """
    ...
    if not os.path.exists(f"{openapi_plugin_client_dir}/ai-plugin.json"):
        ...
        if openapi_spec and not manifest:
            print(f"Generating manifest for {openapi_manifest_url}")
            manifest = generate_openai_manifest_from_openapi_spec(openapi_spec)
    else:
...
def get_openapi_manifests_and_specs(openapi_config: dict[OpenAPIConfig], plugins_dir: str) -> dict:
    """
    ...
    for api_name, api_config in openapi_config.items():
        openapi_plugin_client_dir = f"{plugins_dir}/openai/{api_name}"
        create_directory_if_not_exists(openapi_plugin_client_dir)
        openapi_spec = None
        manifest = None
        if api_config.openapi_spec:
            openapi_spec = get_openapi_spec(api_config.openapi_spec, openapi_plugin_client_dir)
        if api_config.openai_manifest:
            manifest = get_openai_manifest(api_config.openai_manifest, openapi_spec, openapi_plugin_client_dir)
...
def instantiate_openapi_clients(openapi_config: dict[OpenAPIConfig], cfg: Config) -> Optional[dict]:
    ...
    """
    manifests_specs = get_openapi_manifests_and_specs(openapi_config, cfg.plugins_dir)
    if manifests_specs.keys():
        return initialize_openapi_plugins(manifests_specs, openapi_config, cfg)
    return
