...
class QdrantMemory(MemoryProviderSingleton):
    ...
    def __init__(self, cfg):
        ...
        self.index = qdrant_client.Index(host=qdrant_host, api_key=qdrant_api_key, collection_name=qdrant_collection_name)


    #add a new data point to the Qdrant collection
    def add(self, data):
        vector = get_ada_embedding(data) #converts the data into embeddings 
        result = self.index.add_entities([vector], ids=[str(self.index.get_collection_size())])  #store the embeddings to the index
        self.index.flush()
