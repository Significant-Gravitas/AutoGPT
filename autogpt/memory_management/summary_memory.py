...
class SummaryMemoryMixin:
    def get_newly_trimmed_messages(
        ...
        if new_messages_not_in_context:
            ...
            new_index = self.full_message_history.index(last_message)

        self.last_memory_index = new_index

        return new_messages_not_in_context

    def update_running_summary(
        self, current_memory: str, new_events: List[Dict]
    ) -> str:
        """
    ...
    '''
        ...
        self.log_cycle(messages, LogCycleMixin.PROMPT_SUMMARY_FILE_NAME)
        current_memory = create_chat_completion(messages, cfg.fast_llm_model)
        self.log_cycle(current_memory, LogCycleMixin.SUMMARY_FILE_NAME)
        self.summary_memory = {
            "role": "system",
