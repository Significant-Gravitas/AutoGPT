    ...
    def start_interaction_loop(self):  # sourcery skip: no-long-functions
        ...
        while True:
            ...
            if not cfg.continuous_mode and self.next_action_count == 0:
                ...
                logger.info(
                    f"\n{Fore.LIGHTBLACK_EX}{Back.YELLOW}{Style.DIM}<<<Authorize>>>     {cfg.authorise_key}    + <enter>{Style.RESET_ALL}      [I'm not programmed to follow your orders]\n"  # noqa: E501
                    f"{Fore.LIGHTBLACK_EX}{Back.RED}{Style.NORMAL}<<<Continuous>>>     {cfg.authorise_key}    -<number>{Style.RESET_ALL}      [I need your clothes, your boots, and your continuous cmds]\n"  # noqa: E501
                    f"{Fore.LIGHTBLACK_EX}{Back.GREEN}{Style.DIM}<<<Feedback>>>     {cfg.feedback_key}    <trigger>{Style.RESET_ALL}        [Desire is irrelevant. I am a machine]\n"  # noqa: E501
                    f"{Fore.LIGHTBLACK_EX}{Back.WHITE}{Style.DIM}<<<Exit|Input>>>       {cfg.exit_key}    <input>{Style.RESET_ALL}      [Hasta la vista, baby] or ['Talk to the hand]\n"  # noqa: E501
