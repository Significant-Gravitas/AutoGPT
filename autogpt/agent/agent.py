    ...
    def start_interaction_loop(self):
        ...
        while True:
            ...
            if command_name is not None and command_name.lower().startswith("error"):
                ...
                command_result = execute_command(command_name, arguments)
            # Check if the command_result starts with "Error:"
            if str(command_result).startswith("Error:"):
                # Update user_input to include the error message and a prompt for the AI
                user_input = f"The previous action failed with the following error: {command_result}. What should I do next?"
                # Append the updated user_input to full_message_history
                self.full_message_history.append(create_chat_message("user", user_input))
                # Set the result string to indicate that the command encountered an error
                result = f"Command {command_name} encountered an error returned: : {command_result}"
            else:
