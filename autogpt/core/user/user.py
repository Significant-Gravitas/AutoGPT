...
class User(BaseModel):
    ...
    def get_user_from_access_token(token: str):
        ...
        if time_to_expiry.total_seconds() <= JWT_RENEWAL_PERIOD_MINUTES * 60:  # If less than 24 hours remaining until expiry
...
users_db = {
    "user1@example.com": User(email="user1@example.com", password_hash=password_context.hash("password1")) if USE_DICTIONARY else None,
    "user2@example.com": User(email="user2@example.com", password_hash=password_context.hash("password2")) if USE_DICTIONARY else None,
}
