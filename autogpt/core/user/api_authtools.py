...
from autogpt.core.user.middleware_jwt import JWTAuthenticationMiddleware
from autogpt.core.user.user import UserLogin
...
async def password_reset(user: UserLogin, background_tasks: BackgroundTasks):
    ...
    return JSONResponse(status_code=200, content={"message": "Password reset link has been sent to the provided email."})
...
async def email_verify(user: UserLogin, background_tasks: BackgroundTasks):
    ...
    email = MessageSchema(subject="Email Verification", recipients=[user.email], body=link)
    fm = FastMail(conf)
    background_tasks.add_task(fm.send_message, email)
    return JSONResponse(status_code=200, content={"message": "Email verification link has been sent to the provided email."})
