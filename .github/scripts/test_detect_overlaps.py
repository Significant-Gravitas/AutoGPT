...
class OverlapSnapshotRetryTests(unittest.TestCase):
    def test_rebuilds_the_complete_inventory_after_head_movement(self):
        result = ([], {})
        current = object()
        with patch.object(
            detect_overlaps,
            "find_overlapping_prs_once",
            side_effect=[detect_overlaps.OverlapSnapshotChanged("head moved"), result],
        ) as attempt:
            self.assertIs(
                detect_overlaps.find_overlapping_prs(
                    "o", "r", "dev", current, 0, False
                ),
                result,
            )
            self.assertEqual(attempt.call_count, 2)
            self.assertIs(attempt.call_args.args[3], current)

    def test_continuous_head_movement_still_fails(self):
        with patch.object(
            detect_overlaps,
            "find_overlapping_prs_once",
            side_effect=detect_overlaps.OverlapSnapshotChanged("head moved"),
        ) as attempt, self.assertRaises(detect_overlaps.OverlapSnapshotChanged):
            detect_overlaps.find_overlapping_prs("o", "r", "dev", object(), 0, False)
        self.assertEqual(attempt.call_count, 3)

    def test_other_infrastructure_errors_are_not_mislabeled_as_head_movement(self):
        with patch.object(
            detect_overlaps,
            "find_overlapping_prs_once",
            side_effect=detect_overlaps.OverlapInfrastructureError("bad report"),
        ) as attempt, self.assertRaises(detect_overlaps.OverlapInfrastructureError):
            detect_overlaps.find_overlapping_prs("o", "r", "dev", object(), 0, False)
...
class GitHubRetryTests(unittest.TestCase):
    def test_transient_read_failure_is_retried(self):
        failure = completed(stderr="HTTP 502: Bad Gateway", returncode=1)
        success = completed(stdout="ok")
        with (
            patch.object(
                detect_overlaps.subprocess, "run", side_effect=[failure, success]
            ) as run,
            patch("time.sleep") as sleep,
        ):
            self.assertIs(detect_overlaps.run_gh(["api", "repos/owner/repo"]), success)
            self.assertEqual(run.call_count, 2)
            sleep.assert_called_once_with(2)

    def test_exhausted_reads_still_fail(self):
        failure = completed(stderr="HTTP 503: unavailable", returncode=1)
        with (
            patch.object(
                detect_overlaps.subprocess, "run", return_value=failure
            ) as run,
            patch("time.sleep"),
            self.assertRaises(SystemExit),
        ):
            detect_overlaps.run_gh(["pr", "view", "1"])
        self.assertEqual(run.call_count, 3)

    def test_mutations_and_permission_failures_are_not_retried(self):
        for args, error in (
            (["pr", "comment", "1", "--body", "hello"], "HTTP 502"),
            (["api", "graphql", "-f", "query=mutation { example }"], "HTTP 502"),
            (["api", "repos/owner/repo"], "HTTP 403: forbidden"),
        ):
            with self.subTest(args=args):
                failure = completed(stderr=error, returncode=1)
                with (
                    patch.object(
                        detect_overlaps.subprocess, "run", return_value=failure
                    ) as run,
                    ...
                    self.assertIs(detect_overlaps.run_gh(args, check=False), failure)
                self.assertEqual(run.call_count, 1)
                sleep.assert_not_called()

    def test_read_timeout_is_bounded_and_retried(self):
        success = completed(stdout="ok")
        with patch.object(
            detect_overlaps.subprocess,
            "run",
            side_effect=[subprocess.TimeoutExpired("gh", 90), success],
        ) as run, patch("time.sleep"):
            self.assertIs(detect_overlaps.run_gh(["pr", "view", "1"]), success)
...
class PullRequestInventoryTests(unittest.TestCase):
    ...
    def test_changed_rest_snapshot_is_retryable(self):
        for head, count in (("new-head", 2), ("expected-head", 3)):
            with self.subTest(head=head, count=count), patch.object(
                detect_overlaps,
                "run_gh",
                return_value=completed(
                    stdout=json.dumps({"head": {"sha": head}, "changed_files": count})
                ),
            ):
                with self.assertRaises(detect_overlaps.OverlapSnapshotChanged):
                    detect_overlaps.verify_pr_snapshot(
                        "owner", "repo", 17, 2, "expected-head"
                    )
    ...
    def test_query_open_prs_rejects_duplicate_prs_across_pages(self):
        self._check_duplicate_prs_are_rejected()

    def test_query_order_does_not_move_when_pull_requests_are_updated(self):
        with patch.object(
            detect_overlaps,
            "run_gh",
        ...
        query = run_gh.call_args.args[0][-1]
        self.assertIn("field: CREATED_AT, direction: ASC", query)
        self.assertNotIn("field: UPDATED_AT", query)

    def _check_duplicate_prs_are_rejected(self):
...
class DiffParsingTests(unittest.TestCase):
    ...
    def test_old_rename_alias_is_classified_and_formatted_as_medium_risk(self):
        self._check_old_rename_alias_risk()

    def test_rename_paths_with_spaces_preserve_source_alias(self):
        changes = detect_overlaps.parse_diff_ranges(
            "diff --git a/my old.py b/my new.py\n"
            "similarity index 100%\n"
            "rename from my old.py\n"
            "rename to my new.py\n"
        )
        renamed = changes["my new.py"]
        self.assertTrue(renamed.is_rename)
        self.assertEqual(renamed.old_path, "my old.py")
        self.assertTrue(
            detect_overlaps.requires_authoritative_merge_test(
                changes, changes, ["my new.py"]
            )
        )

    def _check_old_rename_alias_risk(self):
...
class MergePlumbingTests(unittest.TestCase):
    def test_ref_mode_fetches_head_sha_from_local_checkout(self):
        current = FindOverlappingPullRequestsTests().make_current(0, "a" * 40)
        with (
            patch.object(detect_overlaps, "clone_repo"),
            patch.object(detect_overlaps, "configure_git"),
            patch.object(detect_overlaps, "verify_fetched_ref") as verify,
            patch.object(
                detect_overlaps,
                "run_git",
                side_effect=[completed(stdout="/source/repo\n"), completed()],
            ) as run_git,
        ):
            detect_overlaps.run_batch_merge_tests("o", "r", "dev", current, [])
        self.assertEqual(
            run_git.call_args.args[0],
            ["fetch", "/source/repo", f"{current.head_sha}:current-overlap-source"],
        )
        self.assertEqual(verify.call_args.args[2], current.head_sha)

    def test_batch_source_fetch_failure_is_fatal(self):
        current = FindOverlappingPullRequestsTests().make_current(42, "a" * 40)
        with (
            patch.object(detect_overlaps, "clone_repo"),
            patch.object(detect_overlaps, "configure_git"),
            patch.object(detect_overlaps, "verify_fetched_ref") as verify,
            patch.object(
                detect_overlaps,
                "run_git",
                return_value=completed(returncode=128, stderr="source unavailable"),
            ),
            self.assertRaisesRegex(
                detect_overlaps.OverlapInfrastructureError,
                "fetch current source PR #42 failed",
            ),
        ):
            detect_overlaps.run_batch_merge_tests("o", "r", "dev", current, [])
        verify.assert_not_called()

    def test_batch_other_pr_fetch_failure_is_fatal(self):
        current = FindOverlappingPullRequestsTests().make_current(42, "a" * 40)
        other = FindOverlappingPullRequestsTests().make_current(43, "b" * 40)
        overlap = detect_overlaps.Overlap(current, other, ["changed.py"], {})
        with (
            patch.object(detect_overlaps, "clone_repo"),
            patch.object(detect_overlaps, "configure_git"),
            patch.object(detect_overlaps, "verify_fetched_ref"),
            patch.object(detect_overlaps, "try_merge_ref") as merge,
            patch.object(
                detect_overlaps,
                "run_git",
                side_effect=[
                    ...
                    completed(),
                    completed(),
                    completed(returncode=128, stderr="other source unavailable"),
                ],
            ),
            self.assertRaisesRegex(
                detect_overlaps.OverlapInfrastructureError,
                "fetch PR #43 for merge test failed",
            ),
        ):
            detect_overlaps.run_batch_merge_tests("o", "r", "dev", current, [overlap])
    ...
    def test_clone_fetches_complete_history_for_merge_testing(self):
        with patch.object(
            detect_overlaps,
            "run_git",
            return_value=completed(),
        ...
        self.assertFalse(any(arg.startswith("--depth") for arg in clone_args))
        self.assertNotIn("--filter=blob:none", clone_args)
        self.assertIn("--single-branch", clone_args)

    def test_temporary_clone_disables_automatic_background_maintenance(self):
        with patch.object(
            detect_overlaps,
            "run_git",
            return_value=completed(),
        ...
        clone_args = run_git.call_args.args[0]
        self.assertIn("--config=maintenance.auto=false", clone_args)
        self.assertIn("--config=gc.auto=0", clone_args)
