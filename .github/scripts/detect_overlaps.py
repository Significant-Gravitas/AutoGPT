...
class OverlapSnapshotChanged(OverlapInfrastructureError):
    """Raised when a PR moves while its inventoried revision is being checked."""
...
def find_overlapping_prs(
    ...
    for attempt in range(3):
        try:
            return find_overlapping_prs_once(
                owner, repo, base_branch, current_pr, current_pr_number, skip_merge_test
            )
        except OverlapSnapshotChanged as error:
            if attempt == 2:
                raise
            print(
                f"PR heads moved; rebuilding overlap inventory: {error}",
                file=sys.stderr,
            )
    raise AssertionError("unreachable")


def find_overlapping_prs_once(
    owner: str,
    repo: str,
    base_branch: str,
    current_pr: "PullRequest",
    ...
    other_prs = [
        ...
        and (current_pr_number != 0 or p["head_sha"] != current_pr.head_sha)
    ]
...
def run_batch_merge_tests(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if current_pr.number:
            ...
            current_refspec = f"{current_pr.head_sha}:{current_local_ref}"
...
def verify_fetched_ref(
    ...
    if expected_head_sha is not None and actual_head_sha != expected_head_sha:
        raise OverlapSnapshotChanged(f"{label} head changed before merge testing")
...
def run_gh(args: list[str], check: bool = True) -> subprocess.CompletedProcess:
    ...
    read_only = (
        args[:2] in (["pr", "view"], ["pr", "list"], ["pr", "diff"])
        or (len(args) == 2 and args[0] == "api")
        or (
            args[:2] == ["api", "graphql"]
            and any(
                arg.startswith("query=") and arg[6:].lstrip().startswith("query")
                for arg in args
            )
        )
    )
    for attempt in range(1, 4):
        try:
            ...
            result = subprocess.CompletedProcess(
                ["gh"] + args, 124, "", "GitHub command timed out after 90 seconds"
            )
        transient = re.search(
            r"HTTP (?:408|429|5\d\d)|timed? out|timeout|TLS handshake|connection reset|temporary failure|could not resolve",
            result.stderr,
            re.IGNORECASE,
        )
        if result.returncode == 0 or not read_only or not transient or attempt == 3:
            break
        print(
            f"Transient GitHub read failure; retrying attempt {attempt + 1}/3",
            file=sys.stderr,
        )
