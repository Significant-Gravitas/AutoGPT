...
def load_all_strategies() -> list[AbstractPromptStrategy]:
    ...
    return planner.StrategiesSet.get_strategies() + routing.StrategiesSet.get_strategies() + usercontext.StrategiesSet.get_strategies()
...
def load_all_strategiesv2() -> list[AbstractPromptStrategy]:
    ...
    for _, module_name, _ in pkgutil.iter_modules(AFAAS.prompts.__path__, AFAAS.prompts.__name__ + '.'):
