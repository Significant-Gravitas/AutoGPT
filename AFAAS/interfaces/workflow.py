...
class BaseWorkflow(BaseModel): #NOTE: Might be merged with Pipeline
    name : str
    description : str
    examples : list[str] = Field(..., description="Examples of tasks that would fit this workflow.", min_items=2)

class DefaultWorkflow(BaseWorkflow):
    name : str = "default"
    description : str = "Suited for tasks requiring thought, planning, or subdivision."
    examples : list[str] =  ["Writing technical documentation for a software project, because it might involves research and structuring information but not actual software development.",
...
class SoftwareDevelopmentWorkflow(BaseWorkflow):
    name : str = "software_development"
    description : str = "For tasks related to software creation or modification, whether in part or whole."
    examples : list[str] =  ["Developing a new feature for an existing mobile app.",
...
class FastTrackedWorkflow(BaseWorkflow):
    name : str = "fast_tracked"
    description : str = "Ideal for simple tasks not needing subdivision."
    examples : list[str] =  ["Writing 'Hello World' into HelloWorld.txt.",
