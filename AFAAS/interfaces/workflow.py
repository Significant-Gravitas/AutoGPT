...
class BaseWorkflow(BaseModel): #NOTE: Might be merged with Pipeline
    ...
    examples : ClassVar[list[str]] = Field(..., description="Examples of tasks that would fit this workflow.", min_items=2)

class DefaultWorkflow(BaseWorkflow):
    name : ClassVar[str]= "default"
    description : ClassVar[str] = "Suited for tasks requiring thought, planning, or subdivision."
    examples : ClassVar[list[str]] =  ["Writing technical documentation for a software project, because it might involves research and structuring information but not actual software development.",
...
class SoftwareDevelopmentWorkflow(BaseWorkflow):
    ...
    description : ClassVar[str] = "For tasks related to software creation or modification, whether in part or whole."
    examples : ClassVar[list[str]] =  ["Developing a new feature for an existing mobile app.",
...
class FastTrackedWorkflow(BaseWorkflow):
    ...
    examples : ClassVar[list[str]] =  ["Writing 'Hello World' into HelloWorld.txt.",
