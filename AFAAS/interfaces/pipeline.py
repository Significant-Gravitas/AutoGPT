...
class Pipeline(BasePipeline):
    def __init__(self, task : Task, agent : RoutingAgent, autocorrection_strategy : AutoCorrectionStrategy = None , autocorrection_post_processing : Callable = None):

        super().__init__()
        self.jobs : list[JobInterface]= []
        #self._loop = agent._loop 
        self._agent : RoutingAgent = agent
    ...
    async def execute(self) -> (Optional[Task],Optional[RoutingAgent]):
        if self.jobs:
