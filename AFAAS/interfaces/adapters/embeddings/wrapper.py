...
class SearchFilter(BaseModel):
    ...
    def check_agent_or_user_id(cls, v, values, **kwargs):
        if (
            ...
            and "user_id" not in v.keys()
            and "plan_id" not in v.keys()
        ):
...
class VectorStoreWrapper(abc.ABC):
    ...
    async def add_document(
        ...
        document: Document,
        document_id: str = RetrievedDocument.generate_uuid(),
    ) -> str:

        if not any(
            key in document.metadata for key in ["plan_id", "agent_id", "user_id"]
        ):
            raise ValueError(
        ...
        document.metadata["checksum"] = hashlib.md5(
    ...
    async def search_from_uri(
        self, query: str, uri: str, search_filters: SearchFilter, nb_results: int = 5
        ...
        search_filters.add_filter(
            "source", Filter(filter_type=FilterType.START_WITH, value=uri)
        )
    ...
    async def get_related_documents(
        ...
        query_filter = self.generate_filters(
            filters=search_filters, document_type=document_type
        )
        ...
        try:
            ...
            LOG.error(
                "Error while searching for information, the agent will continue to work with limited capacities"
            )
            return []
        ...
        if cluster_search:
            ...
            half_nb_results = math.ceil(
                nb_results * similatity_prevalence
            )  # Round up division
            top_similar_documents = sorted_documents[:half_nb_results]
            remaining_results = nb_results - len(top_similar_documents)
            remaining_documents = [
                doc for doc in sorted_documents if doc not in top_similar_documents
            ]
            # clustered_documents = self._get_most_similar_document_each_cluster(remaining_results, remaining_documents)
            clustered_documents = self._get_centrermost_document_from_each_cluster(
                remaining_results, remaining_documents
            )
            return top_similar_documents + clustered_documents

        return documents[:nb_results]

    def _get_centrermost_document_from_each_cluster(
        self, nb_clusters: int, documents: list[RetrievedDocument]
    ) -> list[RetrievedDocument]:
        ...
        for cluster_index in range(nb_clusters):
            distances = np.linalg.norm(
                vectors - kmeans.cluster_centers_[cluster_index], axis=1
            )
    ...
    def _get_most_similar_document_each_cluster(
        self, nb_clusters: int, documents: list[RetrievedDocument]
    ) -> list[RetrievedDocument]:
        vectors = [doc.embeddings for doc in documents]
        kmeans = KMeans(n_clusters=nb_clusters, random_state=42).fit(vectors)
        ...
        for cluster_index in range(nb_clusters):
            ...
            most_similar_document = max(
                cluster_members, key=lambda doc: doc.similarity_score
            )
        ...
        if isinstance(document_type, list):
            if DocumentType.ALL not in document_type:
                filter.update(
                    self._make_filter(
                        "type",
                        Filter(filter_type=FilterType.IN, value=document_type.value),
                    )
                )
            else:
            ...
            if document_type != DocumentType.ALL:
                filter.update(
                    self._make_filter(
                        "type",
                        Filter(filter_type=FilterType.EQUAL, value=document_type.value),
                    )
                )
    ...
    @abc.abstractmethod
    async def get_documents_with_embeddings(
        self,
        query: str,
        k: int,
        include_metadata: bool,
    ...
    async def _get_documents_with_embeddings_fallback_method(
        self,
        query: str,
        k: int,
        include_metadata: bool,
        ...
        for doc in documents:
            doc.embeddings = await self._get_embeddings_from_document_id(
...
class ChromaWrapper(VectorStoreWrapper):
    ...
    async def get_documents(
        ...
        return [
            RetrievedDocument(
                vector_store_wrapper=self, document=doc[0], similarity_score=doc[1]
            )
            for doc in documents
