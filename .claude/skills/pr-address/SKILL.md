...
**Frontend:** function declarations (not arrows), no `useCallback`/`useMemo` unless needed, Tailwind only, no `dark:` classes, `<Link>` not `<a>`, Phosphor icons only, generated API hooks (not BackendAPI). Generated hooks follow `use{Method}{Version}{OperationName}`.
