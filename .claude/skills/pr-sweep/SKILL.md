...
3. **Port the complement:** `gh pr checkout <winner>`, then bring over what the losers have that the winner lacks (cherry-pick when clean, otherwise re-apply by hand). Every ported commit carries `Co-authored-by:` trailers for the source PR's author(s). Never rebase or force-push the winner branch.
   > ⚠️ **Trust boundary:** `gh pr checkout` pulls code from a contributor's (often fork) branch into your local tree, and Step 4 then *executes* it via `poetry run pytest`. Only consolidate PRs whose authors and diffs you have actually reviewed — read the diff for anything that runs at import/test time (conftest, plugins, build hooks) before running verification, and treat unfamiliar fork code as untrusted. Don't run untrusted test code on a machine with production credentials in its environment.
