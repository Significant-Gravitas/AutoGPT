---
name: openapi-regen
description: Regenerate the OpenAPI spec and frontend API client. Starts backend, fetches spec, regenerates typed hooks. TRIGGER when API routes change, new endpoints are added, or frontend API types are stale.
user-invocable: true
metadata:
  author: autogpt-team
  version: "1.1.0"
...
Run in a single shell block so `REST_PID` persists:
