...
SDK mode (`use_claude_agent_sdk=True`) spawns a Claude subprocess — **won't work inside Claude Code**. Set `COPILOT_USE_CLAUDE_AGENT_SDK=false` in `backend/.env` to use baseline mode instead.
