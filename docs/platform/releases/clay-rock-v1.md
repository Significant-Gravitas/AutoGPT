...
- New Expert image uploads are limited to PNG/JPEG/WebP, 5 MB, and require image moderation. A short-lived approval receipt is bound to the authenticated owner and a fresh URL before the image is published; receipt failures leave no uploaded file. Both creation and settings upload paths request appearance review. External URL changes cannot bypass review; unchanged existing URLs remain valid. Rejection or service failure leaves the saved appearance unchanged.
