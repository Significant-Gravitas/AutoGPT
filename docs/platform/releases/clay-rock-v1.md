...
4. Do not run the roster seed as a cosmetic migration: it also synchronizes routines, preloads and other behavior. Its presentation backfill now compares each field with the previous template and guards writes against concurrent edits. Identity rescoping retains the existing recognized-default gate. Database-backed migration tests still need the isolated integration stack.
