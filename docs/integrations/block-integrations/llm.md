...
| model | The language model to use for evaluating the condition. | str | No |
...
| model | The language model to use for the conversation. | str | No |
...
| model | The language model to use for generating the list. | str | No |
...
| model | The language model to use for answering the prompt. | str | No |
| force_json_output | Whether to force the LLM to produce a JSON-only response. This can increase the block's reliability, but may also reduce the quality of the response because it prohibits the LLM from reasoning before providing its JSON response. | bool | No |
...
| model | The language model to use for summarizing the text. | str | No |
